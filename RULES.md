...
> 项目级强制约束。**先读懂下面四条原则——它们是判断一切代码写法的依据；后面的具体规则只是这些原则在常见场景下的落地，不是原则本身。**
...
- **用一次性测试验证，不留测试代码。** 开发时写测试验证本次逻辑正确；项目中不包含测试代码，测试代码不提交仓库。

### 原则二：解决问题 ≠ 完成任务

每次修改先解决眼前的问题，然后至少追问两句：**这样改会不会引入新的 BUG？有没有更高效的解法？** 改一处时先想清楚它的影响半径——不要用"修好这个"换来"搞坏那个"。局部写法服务全局：优先选让整体更清晰、关键路径更高效的方案。
