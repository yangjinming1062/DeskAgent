...
> 项目级强制约束。通用原则对每种语言同等生效；语言专属规则见末尾各小节。
...
覆盖核心功能、关键流程与安全边界，不需要覆盖所有细节——代码本身应该简单到读一遍就能确认正确。

**写测试前自检：删掉这个测试，只读业务代码，我会不会漏掉一个真正的 bug？**
- 会 → 写。线程/并发安全、安全边界、平台差异、降级/熔断链、外部交互、副作用/状态转换、边界值格式协商——这些不运行发现不了。
- 不会 → 别写。`cfg_float({"k": 1.5}, "k") == 1.5` 这种读代码就能确认的测试是噪音，不是保护。

**结构要求**：测试文件按被测模块命名（`test_<module>.py`）；每个函数聚焦一个行为；用 `monkeypatch`/`tmp_path`/`fixture` 隔离副作用；mock 外部依赖，不 mock 被测函数本身。
