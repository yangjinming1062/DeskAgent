...
// Reject anything that isn't http(s) before letting renderer navigate. The
// `docsUrl` field is sourced from Backend settings (Backend PUT /api/env),
// and an unvalidated string here would let a malformed server value run JS
// in the renderer via window.open('javascript:...').
const SAFE_DOCS_URL = /^(https?:\/\/)/i
...
export function EnvVarActionsMenu({
  ...
  const safeDocsUrl = trimmedUrl && SAFE_DOCS_URL.test(trimmedUrl) ? trimmedUrl : undefined
  const hasDocs = Boolean(safeDocsUrl)
        ...
        {hasDocs && (
          <DropdownMenuItem
            onSelect={event => {
              ...
              window.open(safeDocsUrl!, '_blank', 'noopener,noreferrer')
            }}
          >
