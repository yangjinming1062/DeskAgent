...
// Module-level default is the canonical English baseline so any code path
// calling `translateNow` before `I18nProvider` mounts (e.g. module-level
// notifications in tests) gets English keys, not the user's `INITIAL_LOCALE`.
// The provider's `useEffect` overrides this with the user-chosen locale on mount.
