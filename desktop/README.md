...
**STT `silent_fallback`**（`stt.silent_fallback`，默认 `true`）：仅作用于 `auto` 档下"本地跑出了弱/错误结果"的回退。`true`（默认）静默改跑云端，用户无感；`false` 则把本地弱结果直接暴露给 renderer（隐私/成本敏感用户不想偷偷走云端）。"本地引擎整体不可用"（runner 未连 / 工具未注册）仍照常回退云端——那是 `auto` 的核心承诺，不属于"弱结果"，不受此开关影响。TTS 回退保持原样（始终静默）。
