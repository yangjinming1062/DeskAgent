...
class IncrementalExternalStoreThreadRuntimeCore extends ExternalStoreThreadRuntimeCore {
  override __internal_setAdapter(store: ExternalStoreAdapter): void {
    ...
    if (hasUpcomingMessage(isRunning, messages)) {
      // core@0.2 dropped `appendOptimisticMessage`. Synthesize the placeholder
      // as a fully-formed optimistic `ThreadMessage` (metadata.isOptimistic
      // is the new wiring) and let `evictOffBranchOptimisticMessages` clean
      // it up once the real chunk arrives.
