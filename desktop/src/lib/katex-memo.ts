 ...
 *
 * The default `@streamdown/math` plugin runs `rehype-katex` on every markdown
 * commit. During streaming, each new token re-runs KaTeX on EVERY math node —
 * including equations that haven't changed. For math-heavy responses this
 * becomes a major source of jank.
 *
 * This plugin keying math nodes by `(displayMode, value)` and serving them
 * from an LRU cache. Each unique equation only pays the katex cost once.
 *
 * Drop-in replacement for the math slot in streamdown's PluginConfig.
 *
