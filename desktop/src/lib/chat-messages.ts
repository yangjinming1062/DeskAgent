...
import type { SessionMessage, UsageStats } from '@/types/zast'
...
export type GatewayEventPayload = {
  ...
  env_var?: string
  prompt?: string
  // tool.call — round-trip id used to pair result with the originating call
  call_id?: string
  // compression.request — conversation id the backend is asking consent for
  session_id?: string
  // references — file citations from the backend
  items?: Array<{ text: string; url?: string }>
  // cron.trigger — scheduled job that just fired
...
function toolId(payload: GatewayEventPayload | undefined): string {
  return payload?.tool_id || payload?.tool_call_id || payload?.id || payload?.call_id || ''
}
