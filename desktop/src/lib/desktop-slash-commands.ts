...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Zast profile'],
  ...
  ['/version', 'Show Zast Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownZastSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Zast' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZastSlashCommand(normalized)
}
...
function isKnownZastSlashCommand(command: string): boolean {
