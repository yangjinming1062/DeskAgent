...
// surrounding text is full of filename-shaped tokens that URL_RE would mistake
// for domains.
const EXPLICIT_URL_RE = /(?:https?:\/\/|www\.)[^\s<>"'`]+[^\s<>"'`.,;:!?)]/gi

const DOMAIN_RE = /^(?:www\.)?[a-z0-9](?:[a-z0-9-]*\.)+[a-z]{2,}(?::\d+)?(?:[/?#][^\s]*)?$/i
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|zast):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.zastDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.zastDesktop?.openExternal?.(href)
  }
}
