...
export function mediaStreamUrl(path: string): string {
  return `zast-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  ...
  if (!window.zastDesktop?.readFileDataUrl) {
    throw new Error('gatewayMediaDataUrl requires zastDesktop.readFileDataUrl')
  }

  return window.zastDesktop.readFileDataUrl(file)
}
