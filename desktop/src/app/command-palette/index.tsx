...
import { $showSubagentsInSidebar } from '@/store/session'
import { type ThemeMode, useTheme } from '@/themes/context'
import { getZastConfigRecord, listSessions } from '@/zast'

import { ARTIFACTS_ROUTE, NEW_CHAT_ROUTE, sessionRoute, SETTINGS_ROUTE } from '../routes'
...
type NonConfigSettingsLabel = 'about' | 'archivedChats' | 'mcp'
...
export function CommandPalette() {
  ...
  const configQuery = useQuery({
    ...
    queryFn: getZastConfigRecord,
    enabled: open
  })

  const sessionsQuery = useQuery({
    queryKey: ['command-palette', 'sessions', $showSubagentsInSidebar.get()],
    queryFn: () => {
      const includeSubagents = $showSubagentsInSidebar.get()

      return listSessions(200, 1, 'exclude', 'recent', includeSubagents)
    },
    enabled: open
  })
  ...
  const baseGroups = useMemo<PaletteGroup[]>(() => {
    ...
    // Preserve the active skills subtab when jumping to the skills view from
    // the palette, so `?tab=skills&subtab=toolsets` round-trips intact.
    const sub = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('subtab') : null

    const skillsTab = sub ? `${settingsTab('skills')}&subtab=${sub}` : settingsTab('skills')
