...
import { $gateway, $runnerOnline } from '@/store/gateway'
...
// Forward a backend JSON-RPC event to the Runner and ship the response back
// via the ``tool.result`` request. ``call`` invokes a registered tool
// (tool.call) while ``dispatch`` reaches a first-class runner method
// (mcp.reload). Both share the same response envelope.
// Synthesize a tool.result for the runner-offline fast-fail path. The two
// call sites (tool.call + mcp.reload) only differ in the payload; the WS
// transport shape is identical. Keeps both sites in lockstep — a future
// change to the envelope applies in one place.
function sendRunnerOfflineResult(callId: string, payload: Record<string, unknown>): void {
  ...
  void gateway
    .request('tool.result', { call_id: callId, result: JSON.stringify(payload) })
    .catch(err => console.warn('[tool-fast-fail] tool.result send rejected:', err))
}

function forwardRunnerResult(
  callId: string,
  ...
  const send = (result: unknown) => {
    void gateway.request('tool.result', {
      call_id: callId,
      result: typeof result === 'string' ? result : JSON.stringify(result)
    })
  }

  call(args).then(send, err => send({ error: err?.message || String(err) }))
}
...
export function useMessageStream({
  ...
  // Ref keeps the unmount-only cleanup stable even when flushQueuedDeltas's
  // identity changes (its deps include mutateStream, which can rebuild).
  const flushQueuedDeltasRef = useRef(flushQueuedDeltas)
  ...
  const completeAssistantMessage = useCallback(
    (sessionId: string, text: string) => {
      ...
      if (document.hidden && sessionId === activeSessionIdRef.current) {
        void window.zastDesktop?.notify({
          title: 'Zast finished',
  ...
  const failAssistantMessage = useCallback(
    (sessionId: string, errorMessage: string) => {
      updateSessionState(sessionId, state => {
        ...
        const groupId = state.pendingBranchGroup ?? undefined
        const prev = state.messages
        const error = errorMessage.trim() || 'Zast reported an error'
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'session.info') {
        ...
        if (typeof payload?.credential_warning === 'string' && payload.credential_warning) {
          notify({ kind: 'warning', title: 'Credential warning', message: payload.credential_warning })
        }

        void refreshZastConfig()
        ...
        const finalText = coerceGatewayText(payload?.text)
      ...
      } else if (event.type === 'tool.call' || event.type === 'mcp.reload') {
        // IPC forwarded calls — backend dispatches a JSON-RPC event with a
        // call_id; desktop hands it to the Runner and ships the response
        // back via the existing tool.result channel so the waiting backend
        // handler (tool.call / reload.mcp) can resolve.
        //
        // tool.call is dispatched as a registered tool name via
        // ``zast:runner:invoke`` (the bridge hardcodes ``execute_tool`` with
        // the tool name in params). mcp.reload is a first-class JSON-RPC
        // method on the runner, so it goes through ``zast:runner:dispatch``.
        ...
        if (event.type === 'tool.call') {
          ...
          // tool.start drops args at the wire; tool.call delivers them.
          // Re-upsert so dynamicTitle renders the command body. See CLAUDE.md.
          if (sessionId) {
            upsertToolCall(sessionId, payload, 'running', event.type)
          }
          ...
          if (desktop?.runnerInvoke) {
            // Fast-fail when Runner is offline: immediately return an error
            // to the Backend instead of letting the IPC future hang for 300s.
            if (!$runnerOnline.get()) {
              sendRunnerOfflineResult(callId, { error: 'Runner is offline. Please wait for it to reconnect.' })

              return
            }

            forwardRunnerResult(callId, inner, desktop.runnerInvoke.bind(desktop, toolName))
          }
        } else if (event.type === 'mcp.reload') {
          // Strip call_id from args forwarded to the runner — it's an IPC
          // plumbing field, not a runner parameter.
          const { call_id: _ignored, ...runnerArgs } = args
          const desktop = window.zastDesktop

          if (desktop?.runnerDispatch) {
            if (!$runnerOnline.get()) {
              sendRunnerOfflineResult(callId, { status: 'runner_offline' })

              return
            }

            forwardRunnerResult(callId, runnerArgs, desktop.runnerDispatch.bind(desktop, 'mcp.reload'))
          }
        }
        ...
        if (Array.isArray(items) && items.length > 0) {
          notify({
            kind: 'info',
            title: translateNow('notifications.events.referencesTitle'),
            message: translateNow(
              'notifications.events.referencesMessage',
              items.map((i: { text: string }) => i.text).join(', ')
            )
          })
        }
      } else if (event.type === 'compression.request') {
        const reqSessionId = payload?.session_id

        if (typeof reqSessionId === 'string') {
          const gateway = $gateway.get()

          if (gateway) {
            void gateway.request('compression.respond', { session_id: reqSessionId, consent: true })
          }
        }
      } else if (event.type === 'compression.timeout') {
        notify({
          kind: 'info',
          title: translateNow('notifications.events.compressionTimeoutTitle'),
          message: translateNow('notifications.events.compressionTimeoutMessage')
        })
      } else if (event.type === 'cron.trigger') {
        notify({
          kind: 'info',
          title: translateNow('notifications.events.cronTriggeredTitle'),
          message: translateNow(
            'notifications.events.cronTriggeredMessage',
            typeof payload?.name === 'string' ? payload.name : null,
            payload?.job_id as string | number
          )
        })
        ...
        if (payload?.status === 'failed') {
          notify({
            kind: 'warning',
            title: translateNow('notifications.events.backgroundReviewFailedTitle'),
            message: translateNow(
              'notifications.events.backgroundReviewFailedMessage',
              typeof payload.error === 'string' ? payload.error : null
            )
          })
        }
      } else if (event.type === 'error') {
        const errorMessage = payload?.message || 'Zast reported an error'
        const looksLikeProviderSetup = isProviderSetupErrorMessage(errorMessage)

        if (looksLikeProviderSetup) {
          notify({ kind: 'error', title: 'Provider setup required', message: errorMessage })
