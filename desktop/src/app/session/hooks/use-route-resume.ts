...
function rawHashLooksLikeSession(): boolean {
  ...
  return !hash.startsWith('/settings') && !hash.startsWith('/artifacts')
}
...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      const alreadyActive =
        routedSessionId === selectedStoredSessionIdRef.current && routedSessionId === activeSessionIdRef.current
