...
export function useSessionStateCache({
  ...
  const sessionStateBySessionIdRef = useRef(new Map<string, ClientSessionState>())
  ...
  const ensureSessionState = useCallback((sessionId: string, storedSessionId?: string | null) => {
    const existing = sessionStateBySessionIdRef.current.get(sessionId)
    ...
    sessionStateBySessionIdRef.current.set(sessionId, created)
  ...
  const updateSessionState = useCallback(
    (
      ...
      sessionStateBySessionIdRef.current.set(sessionId, next)
