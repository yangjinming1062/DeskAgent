...
import { normalizePersonalityValue, personalityNamesFromConfig } from '@/lib/chat-runtime'
import {
  $currentCwd,
  ...
  setIntroPersonality,
  setShowSubagentsInSidebar
} from '@/store/session'
import { getZastConfig, getZastConfigDefaults } from '@/zast'
...
interface ZastConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useZastConfig({ activeSessionIdRef, refreshProjectBranch }: ZastConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshZastConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getZastConfig(), getZastConfigDefaults().catch(() => ({}))])
      ...
      setShowSubagentsInSidebar(config.display?.show_subagents_in_sidebar === true)
    } catch {
      // Config is nice-to-have; chat still works without it.
    }
  }, [activeSessionIdRef, refreshProjectBranch])

  return { refreshZastConfig, sttEnabled, voiceMaxRecordingSeconds }
}
