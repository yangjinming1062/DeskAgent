...
// bug that drives the REAL useGatewayBoot hook + REAL ZastGateway through a
...
beforeEach(() => {
  ...
  ;(window as { zastDesktop?: unknown }).zastDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { zastDesktop?: unknown }).zastDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForZast) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startZast()'s remote branch awaits waitForZast() for 45s before it
    ...
    ;(window as { zastDesktop?: unknown }).zastDesktop = desktop
    ...
    // After ~45s waitForZast gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Zast backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
