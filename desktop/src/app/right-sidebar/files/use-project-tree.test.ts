...
import type { ZastReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ZastReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { zastDesktop: { readDir: typeof readDir } }).zastDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { zastDesktop?: unknown }).zastDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZastReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZastReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZastReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.zastDesktop is missing', async () => {
    delete (window as unknown as { zastDesktop?: unknown }).zastDesktop
