...
import type { ZastReadDirEntry, ZastReadDirResult } from '@/global'

export type ProjectTreeEntry = ZastReadDirEntry
...
async function gitRootFor(start: string) {
  ...
  if (!cached) {
    cached = window.zastDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.zastDesktop?.readDir || !window.zastDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.zastDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.zastDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: ZastReadDirEntry) {
...
async function filterIgnored(entries: ZastReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZastReadDirResult> {
  if (!window.zastDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.zastDesktop.readDir(dirPath)
