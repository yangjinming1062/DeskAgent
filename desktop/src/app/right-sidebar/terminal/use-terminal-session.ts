...
const ZAST_PATHS_MIME = 'application/x-zast-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ZAST_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ZAST_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.zastDesktop?.terminal
    ...
    try {
      ...
      console.warn('[zast-terminal] WebGL unavailable; falling back to DOM', err)
    }
