...
import { deleteSession, listSessions, setSessionArchived } from '@/zast'
...
export function SessionsSettings() {
  ...
  // i18n strings change on locale switch; capture once-stable ref so the
  // empty-deps `load` callback stays identity-stable across locales.
  const failedLoadRef = useRef(s.failedLoad)
  failedLoadRef.current = s.failedLoad
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      notifyError(err, failedLoadRef.current)
    } finally {
      setLoading(false)
    }
  }, [])
...
// / Program Files), which buried any files Zast wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.zastDesktop` is
    ...
    const settings = window.zastDesktop?.settings
  ...
  return (
    <div className="mb-6">
      ...
      <ListRow
        ...
        description={dir || s.defaultsTo(fallback || '~/zast-projects')}
