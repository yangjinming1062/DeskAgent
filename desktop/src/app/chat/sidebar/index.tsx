...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/zast'

import { type AppView, appViewForPath, ARTIFACTS_ROUTE, INSIGHTS_ROUTE } from '../../routes'
...
const SIDEBAR_NAV: SidebarNavItem[] = [
  ...
  { id: 'artifacts', label: '', icon: props => <Codicon name="files" {...props} />, route: ARTIFACTS_ROUTE },
  {
    id: 'insights',
    label: '',
    icon: props => <BarChart3 className={props.className} />,
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('zast:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('zast:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // Workspace grouping is purely a recents-side concern; ordering stays in sync
  // with the sidebar atom so a group drag persists across refreshes.
  ...
  // Pagination tracks the unified loaded set against the reported total.
  const loadedSessionCount = sessions.length

  const knownSessionTotal = Math.max(sessionsTotal, loadedSessionCount)
  ...
  const displayAgentGroups = agentsGrouped ? agentGroups : undefined

  useEffect(() => {
    if (!displayAgentGroups?.length) {
      return
    }
  ...
  }, [displayAgentGroups, workspaceOrderIds])
      ...
      <SidebarContent className="gap-0 overflow-hidden bg-transparent px-2.5">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-px">
              {SIDEBAR_NAV.map(item => {
                ...
                const active = item.route ? appViewForPath(item.route) === currentView : false
        ...
        {contentVisible && showSessionSections && trimmedQuery && (
          <SidebarSessionsSection
            ...
            contentClassName="flex min-h-0 flex-1 flex-col gap-px overflow-y-auto overflow-x-hidden overscroll-contain pb-1.75"
            emptyState={
        ...
        {contentVisible && showSessionSections && !trimmedQuery && (
          <SidebarSessionsSection
            activeSessionId={activeSidebarSessionId}
            contentClassName={cn(
              'flex min-h-0 flex-1 flex-col gap-px overflow-y-auto overflow-x-hidden overscroll-contain pb-1.75'
            )}
            dndSensors={dndSensors}
        ...
        {contentVisible && <div className="min-h-0 flex-1" />}
      </SidebarContent>
    </Sidebar>
  )
}
