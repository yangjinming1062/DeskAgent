...
import { zastDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, ZAST_PATHS_MIME } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  const at = useAtCompletions({ requestGateway, sessionId: sessionId ?? null, cwd: cwd ?? null })
  ...
  // cold start ("Starting Zast...") from a dropped connection we're trying to
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = zastDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, ZAST_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(ZAST_PATHS_MIME)) {
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ZAST_PATHS_MIME)) {
      return
    }
