...
export function useAtCompletions(options: {
  requestGateway: GatewayRequest | null
  sessionId: string | null
  cwd: string | null
}): { adapter: Unstable_TriggerAdapter; loading: boolean } {
  const { requestGateway, sessionId, cwd } = options
  const enabled = Boolean(requestGateway)
  ...
  const fetcher = useCallback(
    async (query: string): Promise<CompletionPayload> => {
      ...
      try {
        const result = await requestGateway<{ items?: CompletionEntry[] }>('complete.path', params)
