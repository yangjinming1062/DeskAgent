...
// Silence- or mic-loss-driven hard cap on how long a single voice turn runs
// before we force-close the turn and let the model respond with what it has.
...
export function useVoiceConversation({
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      setStatus('listening')
      turnTimeoutRef.current = window.setTimeout(() => void handleTurn(), VOICE_TURN_TIMEOUT_MS)
    } catch (error) {
