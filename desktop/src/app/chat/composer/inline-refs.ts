...
export const ZAST_SESSION_MIME = 'application/x-zast-session'
...
export function writeSessionDrag(transfer: DataTransfer, payload: SessionDragPayload) {
  transfer.setData(ZAST_SESSION_MIME, JSON.stringify(payload))
  transfer.effectAllowed = 'copy'
}

export function dragHasSession(transfer: DataTransfer | null) {
  return Boolean(transfer) && Array.from(transfer!.types || []).includes(ZAST_SESSION_MIME)
}

export function readSessionDrag(transfer: DataTransfer | null): null | SessionDragPayload {
  const raw = transfer?.getData(ZAST_SESSION_MIME)
  ...
  try {
    ...
    return parsed.id ? { id: parsed.id, title: parsed.title || '' } : null
  } catch {
    return null
  }
}

/** Build a `@session:<id>` chip. Value carries the metadata the agent
 * needs to resolve the link (session_search); label shows the friendly title. */
export function sessionInlineRef({ id, title }: SessionDragPayload): InlineRefInput {
  return { kind: 'session', label: title || `chat ${id.slice(0, 8)}`, value: id }
}
