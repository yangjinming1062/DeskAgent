...
import { ZAST_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.zastDesktop.readFileText) {
    try {
      return await window.zastDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'zast:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.zastDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(ZAST_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
