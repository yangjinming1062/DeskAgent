        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.zastDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, reloadRequest = 0, setTitlebarToolGroup, target }: PreviewPaneProps) {
  const { t } = useI18n()
  const copy = t.preview.web
  ...
  useEffect(() => {
    if (target.kind !== 'file' || !window.zastDesktop?.watchPreviewFile || !window.zastDesktop?.onPreviewFileChanged) {
      return
    }
    ...
    const unsubscribe = window.zastDesktop.onPreviewFileChanged(payload => {
    ...
    void window.zastDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.zastDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.zastDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:zast-preview')
