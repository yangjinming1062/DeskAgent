...
export const ZAST_PATHS_MIME = 'application/x-zast-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Zast sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.zastDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(ZAST_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  // i18n strings change on locale switch; capture them in refs so the
  // image-attach callbacks (empty deps, identity-stable across locales)
  // still see fresh translations on every notifyError call.
  const imagePreviewFailedRef = useRef(copy.imagePreviewFailed)
  imagePreviewFailedRef.current = copy.imagePreviewFailed
  const imageAttachFailedRef = useRef(copy.imageAttachFailed)
  imageAttachFailedRef.current = copy.imageAttachFailed
  const clipboardPasteFailedRef = useRef(copy.clipboardPasteFailed)
  clipboardPasteFailedRef.current = copy.clipboardPasteFailed
  ...
  const pickContextPaths = useCallback(
    async (kind: 'file' | 'folder') => {
      const paths = await window.zastDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(async (filePath: string) => {
    ...
    try {
      const previewUrl = await window.zastDesktop?.readFileDataUrl(filePath)
      ...
      notifyError(err, imagePreviewFailedRef.current)
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.zastDesktop?.saveImageBuffer(data, blobExtension(blob))
        ...
        notifyError(err, imageAttachFailedRef.current)

        return false
      }
    },
    [attachImagePath, copy.imageAttach, copy.imageWriteFailed]
  )
  ...
  const pasteClipboardImage = useCallback(async () => {
    try {
      const path = await window.zastDesktop?.saveClipboardImage()
      ...
      notifyError(err, clipboardPasteFailedRef.current)
    }
  }, [attachImagePath, copy.clipboard, copy.noClipboardImage])
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.zastDesktop?.getPathForFile ? window.zastDesktop.getPathForFile(file) : ''
