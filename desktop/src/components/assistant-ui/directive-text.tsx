...
const ZAST_REF_TYPES = ['file', 'folder', 'url', 'image', 'video', 'tool', 'line', 'terminal', 'session'] as const
type ZastRefType = (typeof ZAST_REF_TYPES)[number]
...
const ICON_PATHS: Record<ZastRefType, string[]> = {
  file: [
  ...
  video: ['M3 7a2 2 0 0 1 2 -2h8a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-8a2 2 0 0 1 -2 -2z', 'M15 10l5 -3v10l-5 -3z'],
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ZastRefType] ?? ICON_FALLBACK
...
const ZAST_DIRECTIVE_RE = new RegExp(
  '@(file|folder|url|image|video|tool|line|terminal|session):(' +
    '`[^`\\n]+`' +
    '|"[^"\\n]+"' +
    "|'[^'\\n]+'" +
    '|\\S+' +
...
export const zastDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZAST_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ZastRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ZastRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Zast directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => zastDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zastDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zastDesktop?.readFileDataUrl(id)
