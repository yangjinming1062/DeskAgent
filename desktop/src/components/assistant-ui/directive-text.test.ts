...
import { formatRefValue, zastDirectiveFormatter } from './directive-text'
...
describe('zastDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zastDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zastDirectiveFormatter.parse('@file:src/main.tsx the entry point')
