...
import { extractDroppedFiles, ZAST_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
import { useGatewayRequest } from '@/app/gateway/hooks/use-gateway-request'
import { DirectiveContent, zastDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
const StreamStallIndicator: FC<{ activity: string }> = ({ activity }) => {
  ...
  return (
    <StatusRow className="mt-1.5" data-slot="aui_stream-stall" label="Zast is thinking">
      <span aria-hidden="true" className="dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" />
      <ActivityTimerText seconds={elapsed} />
    </StatusRow>
  )
...
const UserMessage: FC<{
  ...
  const bubbleContent = (
    <>
      ...
      {hasBody && (
        // Render the user's text with directive chips (`@file:` etc.) resolved
        // inside plain-text spans.
...
const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const at = useAtCompletions({ cwd, requestGateway, sessionId })
