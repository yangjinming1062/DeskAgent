...
describe('mergeSessionPage', () => {
  ...
  it('keeps a subagent survivor via its own id even when the page omits subagents', () => {
    // Regression guard for hide-subagents-by-default: a pinned subagent must survive
    // a page that filters by `parent_id IS NULL`.
    const previous = [session({ id: 'sub-pinned', _lineage_root_id: 'parent' })]
    const incoming = [session({ id: 'recent' })]

    const merged = mergeSessionPage(previous, incoming, ['sub-pinned'])

    expect(merged.map(s => s.id)).toEqual(['sub-pinned', 'recent'])
  })
})
