...
describe('subagent store', () => {
  ...
  it('persists session_id from spawn event onto the row', () => {
    upsertSubagent('s1', {
      goal: 'scan files',
      status: 'running',
      subagent_id: 'a1',
    ...
    expect(listFor('s1')[0]?.sessionId).toBe('42')
  })

  it('keeps session_id across progress events that omit it', () => {
    upsertSubagent('s1', {
      goal: 'scan files',
      status: 'running',
      subagent_id: 'a1',
  ...
  it('leaves session_id undefined for delegate_task fallback rows', () => {
    upsertSubagent('s1', {
      goal: 'fallback',
      status: 'running',
    ...
    expect(listFor('s1')[0]?.sessionId).toBeUndefined()
  })
})
