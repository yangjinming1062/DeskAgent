...
const REGISTRY_STORAGE_KEY = 'zast.desktop.sessionPreviews.v1'
const MAX_RECORDS_PER_SESSION = 1
const MAX_SESSIONS = 120
const MAX_FILE_PREVIEW_TABS = 8
...
// Subscriber prunes before persisting so the on-disk shape is always bounded.
// Subscribing here keeps the in-memory atom and the localStorage copy in
// lockstep without relying on every caller to have run the prune themselves.
...
function openFilePreviewTarget(target: PreviewTarget) {
  ...
  if (index === -1) {
    const appended = [...current, tab]
    // FIFO eviction keeps the rail bounded; we drop the oldest tab when a new
    // one would exceed MAX_FILE_PREVIEW_TABS (matches the cap in notifications.ts).
    const trimmed = appended.length > MAX_FILE_PREVIEW_TABS ? appended.slice(-MAX_FILE_PREVIEW_TABS) : appended
    $filePreviewTabs.set(trimmed)
  } else {
    // Replace in place rather than moving to the end — re-opening a file
    // should keep its rail slot, not reshuffle the order the user has built up.
    $filePreviewTabs.set(current.map((item, i) => (i === index ? tab : item)))
  }
...
function persistSessionPreviewRegistry(registry: SessionPreviewRegistry) {
  ...
  try {
    // Subscriber-time prune keeps the on-disk copy bounded even if a caller
    // set the atom with a still-full map (e.g. legacy localStorage on first
    // load). Setter-side prune is for in-memory shape; this is for storage.
...
export function registerSessionPreview(
  ...
  // Subscriber prunes the localStorage copy; prune here too so the
  // in-memory atom stays bounded at MAX_SESSIONS.
