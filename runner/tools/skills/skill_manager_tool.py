...
def _pinned_guard(name: str) -> str | None:
    try:
        if get_record(name).get("pinned"):
            return f"Skill '{name}' is pinned and cannot be deleted by skill_manage. Ask the user to run `zast curator unpin {name}` if they want to delete it. Patches and edits are allowed on pinned skills; only deletion is blocked."
    except Exception:
        pass
    return None
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "New skills go to $ZAST_HOME/skills/; existing skills can be "
        ...
        "will refuse with a message pointing the user to `zast curator unpin <name>`. "
