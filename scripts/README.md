...
单一入口，端到端编排 **runner (uv build wheel) → desktop (electron-builder) → stage → Tauri (installer)**，末尾产出 `release/DeskAgent-{ver}-update.zip`（`Build-UpdateZip`）——这是 desktop `electron-updater` 通道在下次启动时消费的自更新 artifact。
...
- Tauri 2 默认对 `bundle.resources` 缺失文件**报错**；`build_client.{sh,ps1}` 在 tauri build 之前 patch `tauri.conf.json#bundle.resources` 列表，把 `../payload/desktop/.gitkeep` 占位替换为当前 host 的实际 desktop artifact，build 后 restore（git 状态保持干净）
- **跨平台 build 不可行** —— macOS code-sign 必须 mac host，Windows 必须 win host。脚本校验 `host/target` 匹配

**Windows 最终产物是单个 `DeskAgent-Setup-{ver}.exe`**（不是 NSIS wrapper）。NSIS wrapper 会出现"双安装器"问题——NSIS 把 `DeskAgent-Setup.exe` 装到 Program Files，用户还得再手动跑一次。直接发 `DeskAgent-Setup.exe` 用户双击即看到 Tauri 安装 UI。`build_client.ps1` 用 `tauri build --no-bundle` 跳过 NSIS，产物直接拷贝到 `release/DeskAgent-Setup-{ver}.exe`。
