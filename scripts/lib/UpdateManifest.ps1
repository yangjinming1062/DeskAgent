...
# Shared signing + manifest helpers for the Zast update pipeline.
#
# Imported by:
...
function Sign-Manifest {
    ...
    $tmpPayload = Join-Path ([IO.Path]::GetTempPath()) "zast-sign-payload-$PID.tmp"
    $tmpSig = Join-Path ([IO.Path]::GetTempPath()) "zast-sign-sig-$PID.tmp"
    try {
...
function New-RunnerManifest {
    ...
    # directory. We mirror the build layout: `runner/zast_agent-*.whl` is
