...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.zast/.env ] && grep -q "^GITHUB_TOKEN=" ~/.zast/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.zast/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Zast Agent\",
    \"comments\": [
...
```bash
source "${ZAST_HOME}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Zast Agent. Code looks clean — good test coverage, no security concerns."
