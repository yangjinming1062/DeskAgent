...
version: 0.1.0
author: Bihruze (Asahi0x), Zast Agent
license: MIT
...
```bash
mkdir -p ${ZAST_HOME:-~/.zast}/cache/darwinian-evolver && cd ${ZAST_HOME:-~/.zast}/cache/darwinian-evolver
...
```bash
cd ${ZAST_HOME:-~/.zast}/cache/darwinian-evolver/darwinian_evolver \
...
Open `${ZAST_HOME:-~/.zast}/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=${ZAST_HOME:-~/.zast}/skills/research/darwinian-evolver
DE_DIR=${ZAST_HOME:-~/.zast}/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `${ZAST_HOME:-~/.zast}/skills/...` are user-side and fine.
