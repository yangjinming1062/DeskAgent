...
metadata:
  deskagent:
    ...
    requires_toolsets: [terminal, files, agent_delegate_tool]
...
| **Low** (major unknowns) | Ask 1-2 targeted questions in plain text, then draft |
...
### Step 0.5: Create a Progress File

Use `write_file` to create a structured project plan, and `patch` to update it throughout the project:

```python
# Create progress file
write_file("progress.md", """# Research Paper Progress
...
# Later, mark tasks complete:
patch("progress.md", "- [ ] Define one-sentence contribution", "- [x] Define one-sentence contribution")
```

This serves as the persistent state across sessions. Always use `read_file`/`patch`/`write_file` tools to manage it.
...
**For agent-based workflows**: Delegate each round's queries in parallel via `agent_delegate_tool` (call multiple times in the same turn for concurrency). Collect results, deduplicate, then generate the next round's queries from the combined learnings.
...
## DeskAgent Agent Integration

This skill is designed for the DeskAgent agent. It uses DeskAgent tools, delegation, scheduling, and memory for the full research lifecycle.

### Related Skills

Compose this skill with other DeskAgent skills for specific phases:
...
| **plan** | Phase 0 (Setup): creating structured plans before execution. Writes to `$DESKAGENT_HOME/plans/` | `skill_view("plan")` |
...
| **`agent_delegate_tool`** | **Parallel section drafting** — spawn isolated subagents for each section. Also for concurrent citation verification. Call multiple times in the same turn for concurrency. |
| **progress file** (`read_file`/`patch`/`write_file`) | Primary state tracker across sessions. Update after every phase transition. |
...
| **plain-text questions** | Ask the user targeted questions in plain text when blocked (venue choice, contribution framing). The `clarify` mechanism is reserved for backend-internal flows; do not assume a tool call exists. |
...
```
agent_delegate_tool(task_description="Draft the Methods section based on these experiment scripts and configs. Include: pseudocode, all hyperparameters, architectural details sufficient for reproduction. Write in LaTeX using the neurips2025 template conventions.")

agent_delegate_tool(task_description="Draft the Related Work section. Use web_search and web_extract to find papers. Verify every citation via Semantic Scholar. Group by methodology.")

agent_delegate_tool(task_description="Draft the Experiments section. Read all result files in results/. State which claim each experiment supports. Include error bars and significance.")
```

Call `agent_delegate_tool` multiple times in the same turn to run them concurrently. Each delegate runs as a **fresh subagent** with no shared context — provide all necessary information in the prompt. Collect outputs and integrate.
...
### State Management with `memory` and progress file
...
**Progress file** — track granular progress using `read_file`/`patch`/`write_file`:

```python
# Add new tasks
write_file("progress.md", open("progress.md").read() + "\n- [ ] Design constrained task experiments for Sonnet 4.6")

# Or use patch to update status
patch("progress.md", "- [ ] Draft Methods section", "- [x] Draft Methods section")
```

**Session startup protocol:**
```
1. read_file("progress.md")              # Check current task list
...
Ask the user a brief, targeted question in plain text when genuinely blocked:
