...
- A long-running process (deskagent gateway, tui_gateway) misbehaves and you can't restart it
...
For long-lived processes: DeskAgent gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source $DESKAGENT_HOME/runner/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside DeskAgent you have two practical options:
...
```json
{
  "name": "Attach to DeskAgent",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<your-project-root>" }
  ]
}
```
...
## Debugging Subprocess / Worker Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### One-shot CLI / single-process script
Easiest: add `breakpoint()` near the suspect line, then run your script normally. Control returns to your terminal at the pause point.

### Long-running subprocess (TUI / IPC worker / daemon)
When the target process is a child of another long-running parent, the parent must be told to forward stdio (otherwise `pdb` blocks silently). Options:

**A. Source-edit the subprocess entrypoint:**
```python
# near the top of the subprocess's serve() / main()
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
Start the parent; the subprocess will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
```
Trigger the matching event from the parent, then `nc 127.0.0.1 4444` in another terminal.

### Persistent worker subprocess
Same pattern — `remote-pdb` with `set_trace()` inside the worker's `exec` path. The worker is persistent across events, so the first trigger blocks until you connect; subsequent events pass through normally unless you re-arm.
...
8. **Environment isolation.** Debug with raw `pytest` first to repro, then re-confirm under the test wrapper before pushing.

9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For DeskAgent subagents, debug one process at a time.
