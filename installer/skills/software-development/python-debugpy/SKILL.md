...
- A long-running process (zast gateway, tui_gateway) misbehaves and you can't restart it
...
For long-lived processes: Zast gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source $ZAST_HOME/runner/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Zast you have two practical options:
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Zast subagents, debug one process at a time.
