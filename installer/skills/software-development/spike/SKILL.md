...
**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), call `agent_delegate_tool` **multiple times in the same turn** so they run concurrently:

```
agent_delegate_tool(task_description="Build 002a-pdf-parse-pdfjs: ... Available tools: terminal, read_file, write_file, search_files, patch, execute_code.")
agent_delegate_tool(task_description="Build 002b-pdf-parse-camelot: ... Available tools: terminal, read_file, write_file, search_files, patch, execute_code.")
```
