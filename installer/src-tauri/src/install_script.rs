...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $ZAST_SETUP_DEV_REPO_ROOT.
//!      Lets devs iterate on the script without re-bundling.
//!   2. Bundled: Tauri `bundle.resources` (`payload/install.{sh,ps1}`). The
//!      `Zast-Setup` binary is self-contained — no network, no GitHub, no
//!      cache. The script version IS the installer build version.
//!
//! Mirrors `desktop/electron/bootstrap-runner.cjs`'s `resolveInstallScript`,
//! but the dev-checkout resolution is driven by an env var rather than the
//! Electron app's APP_ROOT/.. trick, because Zast-Setup.exe is meant to
...
pub enum ScriptSource {
    /// Loaded from a sibling repo checkout via $ZAST_SETUP_DEV_REPO_ROOT.
    DevCheckout,
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("ZAST_SETUP_DEV_REPO_ROOT") {
...
fn resolve_bundled(resource_dir: &Path, kind: ScriptKind) -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "install script not found in Tauri bundle: {}\n\
         The Zast-Setup binary was built without `payload/{}` in its\n\
