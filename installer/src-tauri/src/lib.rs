...
pub fn run() {
    // Tracing → bootstrap-installer.log under ZAST_HOME/logs/ so install
    ...
    // Zast is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(force_setup, "Zast installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Zast is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Zast" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Zast" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_zast_desktop — out of
            ...
            if cfg!(target_os = "macos") && !force_setup {
                if bootstrap::zast_is_installed() {
                    match bootstrap::spawn_installed_desktop() {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_zast_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "zast already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Zast Setup");
}
