...
生活空间与工作台两个入口的互斥开窗完全由客户端负责。后端仅暴露房间、时刻、日记、会话与工具等资源，不下发窗口像素指令，也不为工作台单独生成工位背景。工作台窗内不挂载形象引擎；桌面精灵在窗口外侧栖息伴工。常规对话生图仅产生会话媒体卡片，绝不触碰激活房间背景；更换房间必须走房间生成接口或专属换房工具。改此处需同步：后端房间端点、房间服务与客户端房间组件。
