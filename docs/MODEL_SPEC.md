...
- **渲染风格按类人面孔分流**：类人面孔的模型以二次元 NPR 为目标效果（文生3D 提示词按同一分流措辞，默认「精美二次元」；客户端 toon 色阶 + 描边 + rim + 面部法线平滑完成风格化，见 client 模块文档）；非人生物保留写实 PBR 路线（无恐怖谷问题）。风格随模型元数据下发，客户端可热切换 NPR ⇄ PBR。
