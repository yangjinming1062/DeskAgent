...
- 引导流程只生成并确认 2D 正面种子图（自然站姿、赛璐珞画风）——它是 2D 分层动画的唯一必需输入，也是形象锁定本体与换装、2D 重切分的身份锚，确认后不再变更。
...
- 3D 种子存独立列、不覆盖 2D 正面种子（两者生命周期不同：2D 种子跨渲染模式长期服务身份锚定，3D 种子服务 3D 生成与向导断点恢复）；形象锁定不约束该路径（姿态/视角派生而非身份变更），锁定后生成的 3D 种子直接落正式资产目录，不经草稿 TTL；3D 正面种子重绘会使已派生的背面种子失效。
