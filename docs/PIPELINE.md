...
- 正面/背面全身图确认后，后端按供应商多视角能力准备模型输入：支持多视角时，用户在引导流程中可预览、微调重绘并确认背面立绘（确认时若已包含背面图则复用，并补齐左 / 右视角种子图后按多图模式提交）；不支持多视角时跳过背面确认，仅提交正面图。
- 同一画风的左 / 右 / 背面种子图在正面微调重绘时复用；换画风确认时重绘背景种子图，避免不同画风混用。
