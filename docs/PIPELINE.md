...
- **头像种子图为唯一身份基准（防迭代失真铁律）**：除了**背部种子图**为了保证与正面立绘的模型/服装形态完全吻合需要以**正面种子图**为参照基准外，**所有图片生成（2D 正面立绘、3D 正面立绘、衣柜换装外观、表情头像、聊天画/拍自己等）恒以头像种子图为唯一参照基准**，严禁使用派生出的全身图作为二阶/多阶基准，从源头杜绝衍生图片产生代际衰减与迭代失真。
