...
async def _finalize_generation(
    model_id: int, user_id: int, *, asset_url: str, provider: str, species: str, rig_type: str, style: str = "cel_shading", content_hash: str = ""
) -> bool:
...
async def _maybe_apply_capability(
    ...
    *,
    ...
    """按 ``capability_attr`` ClassVar 选择是否消费该 hop;不支持或失败回退到当前状态。"""
    ...
    try:
        ...
        await _persist_download_source(model_id, user_id=user_id, task_id=new_job.job_id, assets=result.assets, provider_label=provider.provider_name, rig_type=rig_type)
        ...
        return _PipelineState(provider_task_id=new_job.job_id, glb_bytes=glb_bytes)
    except Exception:
...
async def run_capability_chain(
    *, provider_name: str | None, user_id: int, view_filenames: dict[str, str], species: str, model_id: int, style: str = "cel_shading", retry_only: bool = False
) -> None:
    ...
    try:
        ...
        provider_label = record.provider or _provider_result_label(provider.provider_name, multiview=getattr(record, "style", "") == "realistic")
    ...
    state = await _maybe_apply_capability(
        state,
    ...
    state = await _maybe_apply_capability(
        ...
        provider=provider,
        rig_type=rig_type,
        submit=lambda: provider.start_animate_bind(state.provider_task_id),
    )

    try:
        asset_url = await asyncio.to_thread(save_companion_model, state.glb_bytes, user_id=user_id)
        activated = await _finalize_generation(model_id, user_id, asset_url=asset_url, provider=provider_label, species=species, rig_type=rig_type, style=record.style or style)
...
async def recover_stuck_model_generations() -> None:
    """启动时清理被重启遗弃的行:``generating`` 直接判失败以免挡住新生成;下载中的行已握有付费结果,改判 ``download_failed`` 供用户重试下载。"""
    async with SESSION_LOCAL() as db:
        await db.execute(
...
async def _resume_inflight_pipelines() -> None:
    """web 进程启动时扫仍处于 in-flight 状态的行,以 ``retry_only=True`` 重新接续。"""
    async with SESSION_LOCAL() as db:
