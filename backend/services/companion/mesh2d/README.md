...
2D 形象的 CPU 切分链（分层 PSD 首选链关闭或失败时承接）：流水线把立绘切成 6 个核心物理层（back_hair / body_main / clothing / arm_L / arm_R / front_hair），输出 manifest + 部件 PNG，给客户端的 Three.js SkinnedMesh 用。
...
| `pose_estimator.py` | 视觉 LLM 输出 22 关键点；含 `sanitize_keypoints()` 解剖学约束（对称性 + 中线 + 比例 + 缺关键点回退）|
...
- **工程基线**：6 层物理（不抠 eye/mouth）、`material.depthWrite=false` + `renderOrder` + 骨骼 Z 微偏移、眨眼 / 嘴型走骨骼 scale、`sanitize_keypoints()` 解剖学约束、严守旋转幅度红线（head ±15°、breath 1.0~1.015、jiggle ±5px）。
