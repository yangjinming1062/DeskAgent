"""视觉 LLM prompt 模板 — 部件区域识别与关键点估计的指令模板。"""
...
REGION_DETECTION_SYSTEM_PROMPT = (
    ...
    "- front_hair 只覆盖垂在额前/脸侧的发丝区域，不得包含面部皮肤、眼睛、眉毛、嘴——面部全部像素属于 body_main；front_hair 与 body_main 的 bbox 允许在发际线处少量重叠\n"
    "- back_hair 只覆盖轮廓上明显露在身体外侧的后发发丝\n"
    "- 左/右部件（arm/leg）按画面方位命名：画面左侧为 _L，画面右侧为 _R\n"
