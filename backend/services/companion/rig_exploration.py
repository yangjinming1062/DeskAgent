...
async def run() -> int:
    ...
    prerigcheck_task = await rig_check(text_task)
    prerigcheck_output = await poll_rig_check(prerigcheck_task)
    ...
    rig_task = await rig(text_task, recommended, spec="tripo", model_version=MODEL_VERSION_TRIPO)
    print(f"  rig task_id: {rig_task}")
    print("step 4: poll rig task...")
    rig_result = await poll_task(rig_task, interval=3.0, timeout=900.0)
    model_url = rig_result["output"]["model_url"]
    print(f"  model_url (5-min TTL): {model_url}")

    print(f"step 5: download to {GLB_PATH}...")
    raw = await download_model(model_url)
    ...
    metadata = {
        ...
        "model_url": model_url,
        "model_version_used": MODEL_VERSION_TRIPO,
        "spec_used": "tripo",
