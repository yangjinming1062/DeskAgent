...
async def run() -> int:
    ...
    balance = await tripo_client.account_balance()
    ...
    text_task = await tripo_client.create_text_to_model(
        "a simple cartoon bipedal character, A-pose, neutral T-pose, white background", model_version=tripo_client.MODEL_VERSION_DEFAULT
    )
    print(f"  text_to_model task_id: {text_task}")
    print("  waiting for text_to_model to finish...")
    await tripo_client.poll_task(text_task, interval=2.0, timeout=300.0)

    print("step 2: rig-check...")
    prerigcheck_task = await tripo_client.rig_check(text_task)
    prerigcheck_output = await tripo_client.poll_rig_check(prerigcheck_task)
    ...
    rig_task = await tripo_client.rig(text_task, recommended, spec="tripo", model_version=tripo_client.MODEL_VERSION_TRIPO)
    print(f"  rig task_id: {rig_task}")
    print("step 4: poll rig task...")
    rig_result = await tripo_client.poll_task(rig_task, interval=3.0, timeout=900.0)
    model_url = rig_result["output"]["model_url"]
    print(f"  model_url (5-min TTL): {model_url}")

    print(f"step 5: download to {GLB_PATH}...")
    raw = await tripo_client.download_model(model_url)
    ...
    metadata = {
        ...
        "model_url": model_url,
        "model_version_used": tripo_client.MODEL_VERSION_TRIPO,
        "spec_used": "tripo",
