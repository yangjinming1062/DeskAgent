...
async def aclose() -> None:
    """Close the module-level ``httpx.AsyncClient`` singletons owned by providers.

    Only ``brave_free`` and ``tavily`` hold long-lived clients; ``ddgs`` uses the
    ``ddgs`` library with no persistent client. Called from ``main.py`` lifespan
    shutdown so the owning package keeps its private ``_HTTP_CLIENT`` encapsulated.
    """
    from .brave_free.provider import _HTTP_CLIENT as _brave
    from .tavily.provider import _HTTP_CLIENT as _tavily

    await _brave.aclose()
    await _tavily.aclose()
