    ...
    for arg_name in _FILE_PATH_ARG_NAMES:
        ...
        if tool_name in _WRITE_DENIED_TOOLS and is_write_denied(path):
            return ToolGuardrailDecision(
                ...
                code="write_denied",
                message=(
                    f"Blocked {tool_name}: {path} is in the write denylist (SSH credentials, Zast control plane, .env files). This is a backend-side safety check before runner dispatch."
                ),
                tool_name=tool_name,
            )
