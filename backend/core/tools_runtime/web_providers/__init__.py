...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zast tools``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        tool-registration time and on every ``zast tools`` paint. No network I/O.
        """
    ...
    def get_setup_schema(self) -> dict[str, Any]:
        """Return provider metadata for the ``zast tools`` picker.
