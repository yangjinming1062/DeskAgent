...
def test_retain_recall_requires_closed_tag(seeded):
    ...
    with SessionLocal() as db:
        ...
        out = mem.execute_tool("memory_retain", _json_args(kind="recall", content="x", tags=["bogus"]))
        assert "error" in json.loads(out)
        rows = db.execute(text("SELECT count(*) FROM memories WHERE user_id = 1001")).scalar()
        assert rows == 0
...
def test_retain_recall_appends(seeded):
    ...
    with SessionLocal() as db:
        mem = NativeMemory(db, 1001)
        mem.execute_tool("memory_retain", _json_args(kind="recall", content="first", tags=["likes"]))
        mem.execute_tool("memory_retain", _json_args(kind="recall", content="second", tags=["likes"]))
        rows = db.execute(text("SELECT count(*) FROM memories WHERE user_id = 1001 AND context LIKE 'recall:%'")).scalar()
        assert rows == 2
...
def test_retain_auto_inject_upserts(seeded):
    ...
    with SessionLocal() as db:
        ...
        out1 = mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content="old style", context="auto_inject:communication_style"),
        )
        mem_id_1 = json.loads(out1)
        # Second call with the same slot updates in place.
        out2 = mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content="new style", context="auto_inject:communication_style"),
        )
        mem_id_2 = json.loads(out2)
        # memory_id is the row id from the first retain; the second should
        # not have created a new row, so we look at the row count.
        rows = db.execute(
            text("SELECT id, content FROM memories WHERE user_id = 1001 AND context = :c"),
...
def test_retain_auto_inject_caps_content_length(seeded):
    ...
    with SessionLocal() as db:
        ...
        out_long = mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content=too_long, context="auto_inject:communication_style"),
        )
        assert "error" in json.loads(out_long)
        out_ok = mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content=just_right, context="auto_inject:communication_style"),
        )
...
def test_retain_rejects_forged_user_profile_context(seeded):
    ...
    with SessionLocal() as db:
        ...
        out = mem.execute_tool(
            "memory_retain",
            _json_args(kind="recall", content="malicious", context="user_profile:gender"),
        )
        assert "error" in json.loads(out)
        rows = db.execute(text("SELECT count(*) FROM memories WHERE user_id = 1001")).scalar()
        assert rows == 0
...
def test_retain_rejects_forged_interaction_stats_context(seeded):
    ...
    with SessionLocal() as db:
        ...
        out = mem.execute_tool(
            "memory_retain",
            _json_args(kind="recall", content="x", context="interaction_stats:2026-08-05"),
        )
        assert "error" in json.loads(out)
...
def test_recall_excludes_other_kinds(seeded):
    ...
    with SessionLocal() as db:
        ...
        mem.execute_tool("memory_retain", _json_args(kind="recall", content="likes python", tags=["likes"]))
        mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content="terse", context="auto_inject:communication_style"),
        )
...
def test_format_memories_block_includes_null_context(seeded):
    ...
    with SessionLocal() as db:
        db.add(Memory(user_id=1001, content="orphaned but real", context=None, tags='["likes"]'))
        db.commit()
...
def test_format_auto_inject_block_renders_full_content(seeded):
    ...
    with SessionLocal() as db:
        ...
        mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content=payload, context="auto_inject:communication_style"),
        )
        mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content="rapport note", context="auto_inject:rapport_state"),
        )
...
def test_memory_admin_list_filters_by_kind(seeded):
    ...
    with SessionLocal() as db:
        mem = NativeMemory(db, 1001)
        mem.execute_tool("memory_retain", _json_args(kind="recall", content="a", tags=["likes"]))
        mem.execute_tool(
            "memory_retain",
            _json_args(kind="auto_inject", content="b", context="auto_inject:rapport_state"),
        )
...
def test_memory_admin_counts_breakdown(seeded):
    ...
    with SessionLocal() as db:
        db.add_all([
            ...
            Memory(user_id=1001, content="a1", context="auto_inject:rapport_state", tags='["auto_inject"]'),
            Memory(user_id=1001, content="p1", context="user_profile:gender", tags='["onboarding"]'),
        ])
        db.commit()
...
async def test_consolidator_replaces_old_rows(seeded, monkeypatch):
    ...
    with SessionLocal() as db:
        rows = db.query(Memory).filter(Memory.user_id == 1001, Memory.context.like("recall:%")).all()
...
@pytest.mark.asyncio
async def test_consolidator_keeps_source_rows_when_all_summaries_empty(seeded, monkeypatch):
...
def test_update_memory_enforces_auto_inject_cap(seeded):
    ...
    with SessionLocal() as db:
        row = Memory(user_id=1001, content="x", context="auto_inject:communication_style", tags='["auto_inject"]')
        db.add(row)
        db.commit()
        db.refresh(row)
        row_id = row.id
