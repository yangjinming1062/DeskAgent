...
async def test_model_generate_handler_end_to_end(SessionLocal, monkeypatch):
    ...
    async def _fake_pipeline(provider, user_id, species, model_id, style, *, io_dir=None):
        assert io_dir is not None and io_dir.is_dir()
        calls.append({"provider": provider, "user_id": user_id, "species": species, "model_id": model_id, "style": style})
    ...
    assert await runner.drain_once() == 1
    assert calls == [{"provider": "tripo", "user_id": 1, "species": "人类", "model_id": 9, "style": "anime"}]
    async with SessionLocal() as db:
        assert (await db.get(RenderJob, job_id)).status == "succeeded"


async def test_model_generate_handler_rejects_legacy_image_payload(SessionLocal, monkeypatch):
    from services.companion import model_service
    from services.worker import handlers

    handlers.register()

    async def _fail_pipeline(*_args, **_kwargs):
        raise AssertionError("pipeline must not run for a legacy image payload")

    monkeypatch.setattr(model_service, "run_model_gen_pipeline", _fail_pipeline)
    payload = {
        "provider": "tripo",
        "view_filenames": {"front": "f.png"},
        "species": "人类",
        "model_id": 9,
    ...
    async with SessionLocal() as db:
        row = await db.get(RenderJob, job_id)
        # The raw ValueError is log-only (runner sanitizes row.error to fixed
        # copy); the failed status + the never-run pipeline guard is the contract.
        assert row.status == "failed"
