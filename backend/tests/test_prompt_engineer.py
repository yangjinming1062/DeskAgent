...
@pytest.mark.asyncio
async def test_enhance_portrait_returns_llm_text_verbatim(monkeypatch):
    captured: dict = {}
    ...
    async def _fake_chat(db, user_id, system_prompt, user_payload, **_kw):
        ...
        return "portrait portrait of 小光, 纯白平面背景, digital illustration, ..."
    ...
    out = await prompt_engineer.enhance_portrait_prompt(None, 7, _FakePersona(), style="portrait")
    assert "portrait portrait of 小光" in out
...
@pytest.mark.asyncio
async def test_enhance_portrait_includes_feedback_in_payload(monkeypatch):
    seen: dict = {}
    ...
    await prompt_engineer.enhance_portrait_prompt(None, 1, _FakePersona(), feedback="更长的头发")
    payload = json.loads(seen["user_payload"].split("```json\n", 1)[1].split("\n```", 1)[0])
    assert payload["feedback"] == "更长的头发"


@pytest.mark.asyncio
async def test_enhance_portrait_propagates_missing_llm_config(monkeypatch):
    ...
    with pytest.raises(MissingLlmConfigError):
        await prompt_engineer.enhance_portrait_prompt(None, 1, _FakePersona())


@pytest.mark.asyncio
async def test_enhance_portrait_propagates_llm_runtime_error(monkeypatch):
...
@pytest.mark.asyncio
async def test_enhance_portrait_propagates_empty_chat_response(monkeypatch):
