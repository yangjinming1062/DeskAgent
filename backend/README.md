...
排查对话失败时按需开启的开关。开启后每个 LLM 调用在专用 logger 上按调用粒度输出供应商、模型、请求/响应摘要、延迟与失败原因（字段与默认截断长度见 `llm_debug.py`）。默认关闭——开启会把对话内容原样落到 stdout，仅在复现失败时临时启用。
