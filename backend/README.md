...
- **2D / 3D 双渲染模式与 2D 分层骨骼生成流水线**：2D 与 3D 渲染模式并列为一等公民（默认 2D）。2D 骨骼分层流水线采用 Vision LLM（6 部件区域检测 + 19 关键点估计）结合 CPU 向量化抠图与形态学遮挡补全，构建 19 骨骼层级与图层 Mesh，生成轻量 `manifest.json` 与部件切片 PNG。2D 切分秒级就绪、画风 100% 保真、且零外部非开源依赖。
