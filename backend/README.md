...
- **SSRF 保留段检查默认严格、按部署显式豁免**：fake-ip TUN 代理（Clash 类）把所有域名解析进 198.18.0.0/15，出站客户端与供应商产物的对象存储下载会被 SSRF 保留段检查拦截。命中豁免网段只跳过保留段拒绝，域名黑名单、协议白名单、HTTPS→HTTP 降级拦截、云元数据/CGNAT 拦截无条件保留。为什么默认不豁免：多租户部署里该段同样可能是真实内网，"本部署跑在 fake-ip 代理后"是部署者的知识，豁免必须显式配置。
...
- **2D / 3D 双渲染模式与 2D 分层骨骼生成流水线**：2D 与 3D 渲染模式并列为一等公民（默认 2D）。2D 骨骼分层流水线采用 Vision LLM（6 核心部件区域检测 + 可选衣物/双腿层 + 22 关键点估计，面部点缺测时由 head 锚点合成）结合 CPU 向量化抠图与形态学遮挡补全，构建 25 骨骼层级与图层 Mesh，生成轻量 `manifest.json` 与部件切片 PNG。2D 切分秒级就绪、画风 100% 保真、且零外部非开源依赖。
...
- **换装外观走两段式激活**：确认新装时以**不停用现有激活行**的方式插入 mesh2d 行，激活翻转只发生在管线成功落库的同一事务（确认时立下的自动穿着标记仍真才自动换装，期间手动穿着其他装会清除该标记）——激活查询口径只认"激活 + 成功"，提前翻转会令客户端水合落空、精灵掉蛋。所有外观状态校验与翻转（含管线成功接缝）共用用户级锁，锁外校验会让并发双击确认插出两行切分任务。形象锁定不拦截换装（服装/发型是可换元素而非身份变更，同背面种子先例；五官/物种/性别仍由正面种子主参考锚定）；衣柜首次访问时把当前形象懒合成为第一套外观并回填 mesh2d 行的归属，此后所有激活翻转走统一路径（初始形象的立绘文件归头像行所有，删除该外观不动文件）；外观生成走独立小时级频控（默认 1 套/小时），不设数量上限；着装描述由后台 LLM 生成（失败不阻塞就绪）并注入系统提示词稳定段，附仪态联动引导——人格是行为主驱动、着装只调节表达方式，防止 LLM 被衣着风格带偏脱离人设；戳击/抚摸反应的提示词快照同样携带当前着装。
...
- **全异步数据访问层，单一连接池**：所有会话经同一 asyncpg 池（此前同步池与异步池并存、职责重叠且连接数翻倍，合并后单池 + 一条监听专线）；事件监听独占一条进程生命周期直连，断线自动重连。纪律（async 下的坑）：关系属性必须显式预加载（懒加载运行时直接抛错）；全库时间戳带时区、参数必须 aware；**短事务——会话不跨 LLM await**：回合起点、每个持久化点、工具批处理前后、压缩检查点各自开短会话，后台任务按"读 → LLM → 写"三段各持短会话，LLM 调用以无会话方式执行。
