...
- **夜间自主活动批处理，离线而非在线 agentic**：在用户本地休息窗口（0–5 点）跑批处理流水线（画像推断、记忆整理与衰退、主动规划、自我日记），每阶段独立事务、全空可安全回滚。为什么不作为在线循环：批处理在离线状态也可完成认知深化与记忆优化，不强依赖客户端长连接；次日派发的主动回合在触发时才做在线与打扰检查。流水线触发条件为当日存在任意用户交互（无消息数量下限）；每次执行（含跳过）写入 `nightly_activity_logs` 日志行，管理员页面「活动日志」tab 可查看。规划阶段由 LLM 根据对话上下文自主决定次日主动打招呼的时间与内容，不由关键词或固定时间触发。
...
- **迁移只做安全、可回滚的增量**：每个迁移提供可用的降级路径；需要数据回填时在同一迁移内完成并保证幂等；不写破坏性的默认迁移——改列默认值、删数据、不可逆的类型收紧需拆分，或显式说明风险。
...
- **生活空间房间图（角色入画、换装联动失效与锁保护）**：生活空间房间背景将角色绘制进室内场景（产品形态见 [DESIGN.md](../docs/DESIGN.md)，联动与锁见 [PROTOCOL.md](../docs/PROTOCOL.md)）；性格只进入房间简述装配，因为参考图像素读不出性格。工作台不提供单独工位背景；换装成功且当前着装变化时，服务端自动使旧房间图失效并触发重建，防止视觉穿帮；常规对话生图仅产生会话媒体卡片，绝不触碰激活背景，更换房间必须经由专属房间通道或主动换房工具；主动换房受用户级生成互斥锁、每日频控与打扰档位约束，政策锁定时仅响应用户显式指令或换装联动，角色主动请求一律返回人格化拒绝。
- **时刻与日记分层记忆体系**：面向底层检索的长期记忆向量库与面向用户生活空间展示的叙事切片彻底解耦。时间线时刻记录问候、换装里程碑、房间变换等关键足迹，支持系统在生命周期关键点写入或由角色及用户主动记录，角色主动记录受每日配额约束且工作预设会话中禁止记录生活时刻；日记按用户本地日历日归集，夜间批处理流水线自动依据前日对话、记忆与当日时刻提炼第一人称叙事，用户手工编辑过的日记在夜间整理时采取尾部追加而非覆写策略，批处理静默落库且不弹窗打扰。
