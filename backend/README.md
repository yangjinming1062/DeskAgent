...
- **IPC future 按 `(user_id, call_id)` 二元键寻址 + 30s 断线缓冲期（Grace Period）**：并发用户不共享 future；网关引入 `ReplayBuffer`（容量 500 帧、TTL 60s）与 30 秒断线 Grace Period，短时网络抖动期间后台生成任务与 IPC 状态保持存活，重连时经 `session.resume(last_seq)` 增量补发，超时后由 `_grace_cleanup` 统一回收 dispatcher、未决 future 与 runner 工具。
