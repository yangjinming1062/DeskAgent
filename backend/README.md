...
- **助手推理过程随消息行持久化、只给工作台展示**：供应商产出的独立推理过程写入助手消息行，经会话水合与对话完成事件回给客户端；装配 Responses 输入时不回灌，避免把供应商推理协议形状泄漏进下一轮上下文。为什么与正文分列：推理过程不是对用户说的话，也不应占用陪伴对话的可见气泡。
...
- **渐进式工具披露与未压缩历史继承（保护 Prompt Caching）**：全量工具 Schema 会把请求上下文撑到上万 tokens。系统提示词保持静态，不按回合拼业务域清单。当前可用业务域的短描述写在元工具 Schema 里，请求的初始工具列表只挂该元工具。模型按域或意图检索后就地解锁对应 Schema。连续对话中，新回合从尚未压缩的历史继承已解锁工具，历史消息保持只读，从而同时压低体积并保住前缀缓存。
