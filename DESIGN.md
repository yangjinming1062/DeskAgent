...
- **表达永不空白与渐进式定制**：形象种子图确认后，无 3D 模型的窗口期（生成中/失败/无 key/余额耗尽/换模空挡）由**静态精灵相册**兜底——用户看到的始终是自己的角色（静态立绘按状态/情绪切换），程序化蛋形仅存在于种子图产生之前的引导期与相册完全不可用的绝对兜底；通过用户个性化描述与全身种子图生成定制专属 3D 模型；换装系统（材质预设与 AI 纹理）支持后台热替而无需重新生成模型。
...
**伙伴表达永不空白**（[ARCHITECTURE.md §10 #10](ARCHITECTURE.md) 不变量），按三级级联兜底：

1. **GLB（满体验）**：3D 引擎实时渲染——骨骼动画 + morph 表情覆盖全部状态（§2），无需等待逐状态生成。
2. **静态精灵相册**：形象种子图确认后、GLB 不可用（生成中/失败/无 key/余额耗尽/换模空挡）期间的降级渲染。相册由后端按需懒生成——LLM 以自由语义（状态/情绪/反应的开放式描述，如同自创动画）在已有相册标签中匹配，命中即用，未命中则以种子图为身份参考生成一张新的；每用户一张"等待/切换"图优先就绪（进入降级模式即请求，命中相册零 LLM 成本）。图间 250ms 淡切 + CSS 呼吸保持"活着"的观感；戳/拖/气泡/TTS 全部照常。
3. **程序化蛋形**：仅种子图产生之前的引导期占位 + 相册完全不可用（后端不可达/全部生成被拒）时的绝对兜底（基本体组合 + 正弦驱动呼吸/眨眼/说话浮动）。程序化 3D 兜底蛋的色板与 installer 2D 矢量蛋同源（蛋壳 `0xfff4d6` / `#fff4d6`，发光 `#ffd166`）；`emissiveIntensity` 的 3.4s 呼吸节拍与 `login-glow` (`loginGlowPulse`) 锁步。

portrait 重生不触发模型失效——模型只随物种变更或用户显式请求重生；但头像重生会使相册整体失效（身份锚点变了），客户端本地缓存随之清空。
...
[安装完成]
    ...
    ▼
┌─ 蛋 (Egg) ──────────────────────────────────────────────┐
│  形象种子图产生之前，Client 以"蛋"作为占位形象常驻桌面。 │
...
**两类参考图并存（不互斥）**：Q4「基础形象」上传的参考图是**身份锚点**——用户最想要的形象，每次重生都携带；头像重生时可额外附一张**风格参考图**（`/avatar/from-image` 的 `presentation_image` 字段），只影响展现形式（构图、风格等），不替换身份锚点。理由：用户最初选择的形象图承载最强烈的意图，后续的图只是在"怎么呈现"上给参考，两者语义不同故应共存而非覆盖。仅多参考图供应商（MiniMax / Gemini）同时消费两张图；单参考图供应商（Grok）退化为只用身份锚点。

头像确认后进入步 2 链式参考的全身生成。生成行为由 `[companion] fullbody_mode` 配置决定：

- **单视图模式**（`"single"`，默认）：仅生成正面全身立绘（`view="front"`），以确认的头像作为参考图与锚点。正面确认后直接进入音色子阶段。3D 模型经 Tripo3D `image-to-model` 单图生成。共 2 张图（头像 + 正面全身）。
- **多视图模式**（`"multi"`）：逐视角全身生成——每个视角单独生成、单独展示、单独确认，用户可针对任一视角反馈重绘：
  ...
  4. **视图维护规则**：每个视角均可单独重绘（`view="front"|"right"|"back"`）；重绘正面会清空并失效侧面/背面图，要求基于新正面重新生成。三视角全部就绪后作为后续 3D 建模的输入。共 4 张图。

单视图模式选择理由：AI 生成的三视图之间难以保证服装/外貌一致性，不一致的多视图反而降低 3D 模型质量。单张高质量正面图经 Tripo3D `image-to-model` 生成的模型质量更优。多视图模式保留为后续可选升级路径。

**形象确认后锁定**：全身图确认时点击「确认」即视为锁定形象——此后 3D 模型、头像、全身图重新生成路径对当前用户关闭；`PUT /api/companion/persona` 客户端会自动剥离 `biological_type` / `gender` / `appearance_core` 三个视觉字段；名称、性格、说话风格、关系定位、`appearance_outfit`（换装偏好描述）、音色与 user_* 仍可编辑。换装（`POST /api/companion/wardrobe`）与动画生成（`POST /api/companion/animations/generate`）不受影响。**用户责任约束**：图生 3D 模型成本高（Tripo3D image-to-model/multiview-to-3D + rig），且形象一旦进入模型即永久冻结；onboarding 进入任一锁定字段时顶部 banner 直接提示「当前字段是形象确认后无法再次更改的重点内容」，强迫用户在初始引导阶段仔细选择。锁定在客户端剥夺提交能力实现——后端不做 schema 列、不加守卫、不加异常类型。
...
形象 + 音色就位，3D 模型经 `POST /api/companion/model` 异步生成，以全身种子图为输入（单视图模式下仅正面图，多视图模式下三视角图），就绪后经 `model.ready` 事件下发。形象以 idle 骨骼动画"活"起来，用确认后的音色说出第一句问候。onboarding 结束，进入持续陪伴。**形象自此由客户端剥夺了对 3D 模型、头像、全身图以及 persona 中 `biological_type` / `gender` / `appearance_core` 的修改路径；用户可通过换装或编辑 `appearance_outfit` 调整外观**。
...
| **资产缺失（身）** | 无症状 | GLB 不可用时进入静态精灵降级模式（§1.2 级联），用户看到自己的角色静态立绘，完全无感；相册不可用才落蛋形 |
