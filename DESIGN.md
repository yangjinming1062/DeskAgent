...
- **角色子阶段**（Q1–Q7）：名字 → 物种 → 角色性别 → 形象描述 → 角色定位 → 性格 → 说话风格。物种 / 性别 / 形象描述三步连排形成视觉锚点；说话风格同属精灵属性，与其他角色字段一起在孵化时 finalize。
- **音色子阶段**（Q8）：音色偏好（描述句，喂给 §5.5 标签评分）。排在**形象确认（§5.4）之后**——形象与音色都是"精灵长什么样、听起来什么样"的一部分。
...
孵化在角色子阶段（Q1–Q7）答完即触发——不等 13 题收齐。说话风格随其他角色字段一同经 `PUT /api/companion/persona` finalize；finalize 之后 `onboarding.submit` 只接受 `voice` 与 `user_*`，说话风格的后续修改走设置里的角色管理（§8）。

音色描述（Q8）和用户子阶段（Q9–Q13）通过 `onboarding.submit` 逐字段增量提交。
...
音色子阶段紧接形象确认，包含两步：**先**录入 Q8 描述句（"温柔的少女音、节奏偏慢…"），**后**浏览目录。描述提交后 `tts.match_voice {preference}` 把描述经标签评分映射到当前 TTS provider 目录中最贴合的 voice id。目录页形态：
