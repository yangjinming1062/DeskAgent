...
- **角色子阶段**（Q1–Q6）：名字 → 物种 → 角色性别 → 形象描述 → 角色定位 → 性格。物种 / 性别 / 形象描述三步连排形成视觉锚点。
...
**断点恢复**：每个回答经 `onboarding.submit {field, value}` 即时落盘单个字段（见 [PROTOCOL.md §1.1](PROTOCOL.md)），Client 启动时调 `onboarding.get_state` 从下一个未答问题恢复——崩溃/退出不丢进度。**关键差异**：`is_complete=True` 在角色子阶段答完即被设置，但 `get_onboarding_state` 只在 user_* + voice 全部答齐后才返回 `complete=True`；中途崩溃后 client 会恢复进入用户子阶段而不是直接跳到 onboarding 结束。
