...
test('a different voice or text is a different entry', async () => {
  ...
  assert.equal(await cache.read({ language: 'zh', text: 'hi', voice: '茉莉' }), null)
  assert.equal(await cache.read({ language: 'zh', text: 'hi!', voice: '冰糖' }), null)
  assert.equal(await cache.read({ language: 'en', text: 'hi', voice: '冰糖' }), null)
})
...
test('an oversized file on disk reads as a miss', async () => {
  ...
  assert.equal(await cache.read({ language: 'zh', text: 'bloated', voice: 'v' }), null)
})
...
test('write evicts the oldest entries once the cap is exceeded', async () => {
  ...
  await cache.write({ buffer: Buffer.from('new'), language: 'zh', mimeType: MP3, text: 'newest', voice: 'v' })
