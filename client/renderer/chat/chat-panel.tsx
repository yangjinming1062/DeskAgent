...
interface GatewayVisual {
  text: string
  color: string
  dot: string
}

function getGatewayVisual(state: ConnectionState): GatewayVisual {
  if (state === 'open') {
    return {
      text: '在线',
  ...
  if (state === 'connecting') {
    return { text: '连接中', color: 'text-amber-400', dot: 'bg-amber-400' }
  }

  return { text: '未连接', color: 'text-white/40', dot: 'bg-white/30' }
}
...
export function ChatPanel({
  ...
  const gatewayVisual = getGatewayVisual(gatewayState)
    ...
    <div className={cn('flex flex-col flex-1 h-full min-h-0', className)}>
      <div className={headerWrapClass}>
        <div className="flex items-center gap-2">
          ...
          >
            <span className={cn('size-1.5 rounded-full', gatewayVisual.dot)} />
            <span>{gatewayVisual.text}</span>
          </span>
        </div>
        <div
