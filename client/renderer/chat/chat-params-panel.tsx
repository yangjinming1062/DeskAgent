...
import { $chatSessionId, $sessionSettings, updateSessionSetting } from '@/chat/chat-store'
...
export function ChatParamsPanel(): React.JSX.Element {
  ...
  // 累积待同步的增量 patch，避免全量回推与兄弟状态陈旧覆盖
  ...
  // 会话切换时：若上一会话有 pending 变更，在切换前立即 flush，避免污染新会话
  useEffect(() => {
  ...
  // 组件卸载时（例如拖完滑块立即关闭面板），立即 flush 提交未完成的修改，绝不丢失
  useEffect(
    () => () => {
      flushPending()
    },
