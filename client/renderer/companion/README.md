...
- **布料求解器**（[ClothSolver.ts](3d/ClothSolver.ts)）：`physics=cloth` 的 garment 顶点数据由 CPU verlet 接管——顶带锚环（高度前 30%）逐帧钉到 CPU 蒙皮目标随骨骼，其余顶点重力 + 距离约束（索引缓冲去重边）+ 身体骨骼球碰撞（Hips/Spine/腿等后缀匹配，半径表内置）。cloth mesh 渲染为普通 `THREE.Mesh`（渲染器只对 SkinnedMesh 做 GPU 蒙皮），CPU 写入的 position 属性即最终渲染。CPU 蒙皮与 GPU `skinning_vertex` 同公式（[skinning.ts](3d/skinning.ts) 的 `cpuSkinPoint` 显式乘 bindMatrix）。`physics=skin` 单元同样走 CPU 蒙皮（全顶点逐帧钉住）并对**身体表面**碰撞推挤（[BodyCollider.ts](3d/BodyCollider.ts)：顶点聚类代理 + 每帧 CPU 蒙皮 + 中位切分 BVH 最近点推出），动画期防穿模由运行时身体碰撞保证。取舍：cloth 碰撞体是骨骼球而非身体 mesh；顶点预算 16k 上限；几何拟真度天花板。
