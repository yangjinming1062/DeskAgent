...
- **步态与趴姿 overlay**（[gait.ts](gait.ts)）：步态相位按逐帧位移积分而非墙钟推进——easeInOut 减速段步频自然放缓、零脚滑，这是选积分不选定频正弦的原因；躯干起伏/侧摆/朝向微倾以加法叠加在基线上。贴边趴姿是持续 overlay，与动作包络的优先级为「瞬时手势在 tick 末段覆盖其触及的通道、播完自然回落」，二者不互斥。走路/趴姿期间待机呼吸幅度衰减（idleScale），避免呼吸与步态拍频打架。
