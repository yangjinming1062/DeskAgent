...
- **走路 / 跳跃为复合躯干方案**：下半身在 `body_main.png` 内无法独立旋转，walk/walk_fast 用 `body_main` 左右倾斜 + 上下 bob + `shoulder_L/R` 反向摆动 + `skirt/back_hair` 持续 impulse；jump 用 body_main 短暂 squash + shoulder 上扬脉冲。详见 [PROTOCOL.md §1.4 action 白名单](PROTOCOL.md) 的注释。
- **降级链路**：avatar 未确认 → 程序化蛋（`3d/CharacterController.createProcedural`）。avatar 已确认但 2D 不可用 → 程序化蛋。

## 与 SpriteStage 集成

`client/renderer/companion/root.tsx` 根据 `$renderMode` 挂 `<Mesh2DCanvas>` 或 `<Companion3D>`，二者互斥不共存。SpriteStage 的拖拽 / 戳 / 双击 / 鼠标穿透逻辑同时生效。子区域命中由 `mesh2d-hitmap.ts` 提供，`sprite-stage.tsx` 在 pointermove / tap 时调用并把 region 透传给 `handlePokeInteraction`。
...
`backend/services/companion/mesh2d/manifest_exporter.py` 输出的 `animations` 字段除原有 breath / blink / idle_sway / jiggle 外，新增：

- `actions`: 程序化骨骼 pose 表（snake_case key → `bones: { bone_name: { rotation_rad, scale, position_offset } }`）。白名单见 [backend/services/companion/mesh2d/prompts.py:action_prompt_section()](backend/services/companion/mesh2d/prompts.py)。
- `idle_variants`: idle 时按权重随机切换的 pose key 列表。
- `locomotion`: `still / walk / walk_fast / fly / drag / jump` 六种 locomotion 的骨骼相位公式与 jump 脉冲参数。
