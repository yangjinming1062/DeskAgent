...
- **工作台窗外侧伴工由主进程统一广播窗口坐标**：工作台窗口的屏幕坐标由入口窗管理器在显示、移动或调整尺寸时即刻并合帧广播，桌面精灵收到坐标后切到伴工模式并贴在工作台窗外侧（左侧优先、触发贴边导致展示不全自动平滑翻转至右侧，移动时保持恒定距离与锁步协同）。不依赖焦点是否落在工作台——桌面精灵始终跟随工作台物理窗口走。
