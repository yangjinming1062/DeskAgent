...
- **网关单连接与跨窗口宿主-代理模式**：后端协议约束单用户仅允许一条活跃 WebSocket，开新连接会顶掉既有连接；故物理连接收拢于桌面精灵宿主窗口（`mainWindow`）。生活空间与工作台作为独立 BrowserWindow 互斥开窗，启动时挂载网关代理（`IpcGatewayProxy`），经主进程 IPC 双向通道转发 RPC 请求并订阅服务端推送，共享单一链路与对话流；前台表面窗口拥有语音与交互表现，后台宿主窗口抑制重复语音并独占执行 Runner 本机工具。为什么不在各表面窗口开独立 WS：后端连接互斥契约决定多开必踩顶号断连；为什么不在主进程持有 WS：主进程持有需复刻整套重连、心跳与序列号去重逻辑，渲染层透明代理让协议状态与重试机制收敛在单一宿主。
