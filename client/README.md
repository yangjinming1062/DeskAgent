...
- **无边框透明窗口与自定义控制契约（消除系统原生白标题栏）**：工作台与生活空间窗口统一采用无边框透明外壳（`frame: false`, `transparent: true`, `backgroundColor: '#00000000'`），Windows 下启用原生亚克力材质（`backgroundMaterial: 'acrylic'`），macOS 下启用 `vibrancy: 'under-window'`。顶栏内置原生平替样式的轻量窗口控制按钮（最小化、最大化/还原、关闭），通过强类型 IPC 通道（`spiritagent:surface:minimize`、`spiritagent:surface:maximize`、`spiritagent:surface:is-maximized`）由主进程统一调度，支持顶栏双击最大化与拖拽。
...
- **工作台单一复合视窗一体化架构**：工作台采用单一 BrowserWindow 复合视窗架构，左侧内嵌活体伴工精灵插槽，与右侧工作台面板处于同一 DOM 树与物理窗口内。拖拽精灵插槽（原生 `-webkit-app-region: drag`）或工作台标题栏均实现 0 延迟硬件级整体移动，物理杜绝跨窗口位置漂移与忽远忽近问题；工作台开启期间桌面独立精灵舞台收起，工作台关闭时恢复。
- **运行轨迹与对话变体均为客户端纯投影**：工作台右栏运行轨迹与两端对话变体（生活空间弱化实色气泡、工作台指挥台与折叠工具芯片）均由对话状态派生，不另起后端；角色皮肤在确认半身像就绪后于客户端直接由画布提取主色、辅色与高光，注入界面主题变量。换装或角色重生后自动触发重新提取，并以秒级平滑过渡。
