...
- **工具窗与伙伴设置已合并为单一设置面板**：13 个 tab 在 sprite 内的 FloatingPanel 承载（侧栏 nav），居中呈现 960×640 静态面板，由 interactive region 接管鼠标交互与桌面穿透，主窗口保持全屏透明工作区常驻。两条入口——右键菜单的「设置」项、聊天坞顶栏「设置」钮——都改走 `requestOpenDock('settings')`，通知 deep-link（音色失效）也复用同一收口。
