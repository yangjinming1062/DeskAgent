...
- **精灵窗口单显示器跟踪 + 跨屏拖拽接力**：透明精灵窗口同一时刻只覆盖一块显示器（bounds 贴当前显示器 workArea；分辨率变化原地重贴，显示器被拔掉时 `getDisplayMatching` 自动落回最近屏）。拖拽中指针坐标越出视口即光标已跨入邻屏——渲染层经 `spiritagent:sprite:move-to-cursor-display` 请主进程按 `getCursorScreenPoint()` 把窗口 setBounds 到光标所在屏，并用返回的新旧窗口原点差平移拖拽基准（`spatial` 的 resize 处理在 drag 中跳过重定位），精灵在全局桌面原地无跳变。拖拽结束时主进程把窗口原点随位置一并写入 `companion-position.json`，下次启动窗口先贴回精灵所在的显示器再恢复位置。**为什么不做一个覆盖整个虚拟桌面的窗口**：全桌面合成层 + 跨屏 DPI 差异的坐标/命中测试复杂度远高于单屏窗口接力，且 `setIgnoreMouseEvents` 的穿透范围会被迫覆盖所有屏。
