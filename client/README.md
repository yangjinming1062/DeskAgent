...
- **运行轨迹与对话变体均为客户端纯投影**：工作台右栏运行轨迹与两端对话变体（生活空间弱化实色气泡/语音条、工作台指挥台与折叠工具芯片）均由对话状态派生，不另起后端；工作台助手气泡可折叠展示本轮推理过程，生活空间不展示。角色皮肤在确认半身像就绪后于客户端直接由画布提取主色、辅色与高光，注入界面主题变量。换装或角色重生后自动触发重新提取，并以秒级平滑过渡。
- **生活空间始终语音是客户端投影**：始终语音不改协议、不向后端要音频；生活空间代理窗在气泡收尾后拉取一次性 TTS 并落盘，再投影为可点按语音条。产品规则见 [DESIGN.md §6.1](../docs/DESIGN.md)。
