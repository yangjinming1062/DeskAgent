#!/usr/bin/env python3
"""
Procedural rigged GLB model generator for DeskAgent.

Generates 5 species base models (human, elf, spirit_beast, mecha,
shapeshifter) with:
  - Mixamo/HumanIK-compatible humanoid skeleton (~25 bones)
  - ARKit-compatible facial morph targets (~17)
  - Body-shape morphs (~6)
  - PBR materials with embedded procedural textures
  - idle / speaking / thinking / wave animation clips

Run:
    blender --background --factory-startup --python generate_base_models.py

Output:
    backend/assets/base-models/{human,elf,spirit_beast,mecha,shapeshifter}.glb
    backend/assets/base-models/character.glb  (copy of human.glb, generic fallback)
"""
...
OUTPUT_DIR = os.path.join(
    ...
    "backend", "assets", "base-models",
)

SPECIES = {
    "human": {
        "height_scale": 1.0, "width_scale": 1.0,
        "skin_color": (0.85, 0.70, 0.56), "eye_color": (0.10, 0.10, 0.14),
        "skin_roughness": 0.60, "skin_metallic": 0.0,
        "emissive": None, "transmission": 0.0,
        "ring_segments": 16, "subsurf": 1,
    },
    "elf": {
        "height_scale": 1.12, "width_scale": 0.88,
        "skin_color": (0.90, 0.83, 0.75), "eye_color": (0.08, 0.22, 0.38),
        "skin_roughness": 0.55, "skin_metallic": 0.0,
        "emissive": None, "transmission": 0.0,
        "ring_segments": 18, "subsurf": 1,
    },
    "spirit_beast": {
        "height_scale": 0.93, "width_scale": 1.08,
        "skin_color": (0.40, 0.28, 0.18), "eye_color": (0.65, 0.38, 0.10),
        "skin_roughness": 0.75, "skin_metallic": 0.0,
        "emissive": None, "transmission": 0.0,
        "ring_segments": 16, "subsurf": 1,
    },
    "mecha": {
        "height_scale": 1.15, "width_scale": 1.12,
        "skin_color": (0.42, 0.44, 0.48), "eye_color": (0.0, 0.85, 1.0),
        "skin_roughness": 0.35, "skin_metallic": 0.85,
        "emissive": (0.0, 0.7, 0.9), "transmission": 0.0,
        "ring_segments": 6, "subsurf": 0,
    },
    "shapeshifter": {
        "height_scale": 1.0, "width_scale": 0.95,
        "skin_color": (0.78, 0.73, 0.83), "eye_color": (0.55, 0.38, 0.78),
        "skin_roughness": 0.30, "skin_metallic": 0.0,
        "emissive": (0.35, 0.25, 0.55), "transmission": 0.20,
        "ring_segments": 20, "subsurf": 1,
    },
}

# Ring data: (z, radius_x, radius_y[, y_shift]) pre-scaling.
# Character faces -Y in Blender (becomes +Z in glTF Y-up export).
TORSO_RINGS = [
    (0.80, 0.140, 0.100, 0.000), (0.85, 0.150, 0.110, 0.000),
    (0.91, 0.130, 0.090, 0.000), (0.98, 0.140, 0.095, 0.000),
    (1.05, 0.150, 0.100, 0.000), (1.12, 0.160, 0.105, 0.000),
    (1.18, 0.150, 0.100, 0.000), (1.22, 0.100, 0.070, 0.000),
    (1.26, 0.055, 0.050, 0.000), (1.29, 0.050, 0.045, 0.000),
    (1.32, 0.070, 0.065, -0.005), (1.35, 0.082, 0.075, -0.010),
    (1.38, 0.090, 0.082, -0.014), (1.41, 0.096, 0.086, -0.012),
    (1.44, 0.098, 0.090, -0.010), (1.47, 0.096, 0.088, -0.006),
    (1.50, 0.092, 0.085, -0.002), (1.53, 0.082, 0.078, 0.000),
    (1.56, 0.065, 0.062, 0.000), (1.59, 0.040, 0.038, 0.000),
    (1.62, 0.015, 0.014, 0.000),
]
ARM_RINGS = [
    (1.16, 0.048, 0.045), (1.10, 0.046, 0.043), (1.04, 0.044, 0.041),
    (0.98, 0.042, 0.039), (0.92, 0.040, 0.037), (0.86, 0.036, 0.034),
]
LEG_RINGS = [
    (0.80, 0.065, 0.060), (0.74, 0.063, 0.058), (0.66, 0.060, 0.055),
    (0.56, 0.056, 0.052), (0.46, 0.052, 0.048), (0.36, 0.048, 0.045),
    (0.26, 0.044, 0.042), (0.16, 0.040, 0.038), (0.08, 0.036, 0.034),
]
FOOT_RINGS = [
    (0.08, 0.036, 0.034, 0.000), (0.06, 0.038, 0.048, -0.020),
    (0.03, 0.036, 0.044, -0.050), (0.01, 0.028, 0.032, -0.070),
...
def build_tube(bm, specs, segments, h, w, x_center=0.0):
    ...
    for spec in specs:
        ...
        for i in range(segments):
            ...
            ring.append(bm.verts.new((rx * math.cos(ang) + x_center * w,
                                       ry * math.sin(ang) + ys, z)))
        rings.append(ring)
...
def add_face_features(obj, config):
    h = config["height_scale"]; w = config["width_scale"]
    ez = 1.44 * h; ey = -0.085 * w; ex = 0.034 * w
    ...
    for v in bm.verts:
        ...
        for sx in (-ex, ex):
            ...
            if d < 0.030:
                f = falloff(d, 0.030); v.co.y += 0.010 * f
        nc = Vector((0, -0.094 * w, 1.41 * h)); d = (co - nc).length
        if d < 0.020:
            f = falloff(d, 0.020); v.co.y -= 0.012 * f; v.co.z += 0.002 * f
        mc = Vector((0, -0.090 * w, 1.38 * h)); d = (co - mc).length
        if d < 0.024:
            f = falloff(d, 0.024); v.co.y += 0.006 * f
    bmesh.ops.recalc_face_normals(bm, faces=list(bm.faces))
...
def build_armature(config):
    ...
    for name, head, tail, parent_name, _c in bone_defs(h, w):
        ...
        eb.head = Vector(head); eb.tail = Vector(tail); eb.use_connect = False
...
def create_eye_objects(config, armature):
    h = config["height_scale"]; w = config["width_scale"]
    ez = 1.44 * h; ex = 0.034 * w; ey = -0.068
    eyes = []
    for name, xp in [("LeftEye", -ex), ("RightEye", ex)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            radius=0.020, location=(xp, ey, ez), segments=16, ring_count=12)
        eye = bpy.context.object; eye.name = name
        vg = eye.vertex_groups.new(name="Head")
        vg.add(range(len(eye.data.vertices)), 1.0, "REPLACE")
        mod = eye.modifiers.new("Armature", "ARMATURE"); mod.object = armature
        eye.parent = armature; eyes.append(eye)
    return eyes
...
def assign_weights(obj, armature):
    ...
    sigma = 0.075; two_s2 = 2.0 * sigma * sigma
    for v in obj.data.vertices:
        ...
        weights.sort(key=lambda x: -x[1]); weights = weights[:4]
...
def add_morph_targets(obj, config):
    h = config["height_scale"]; w = config["width_scale"]
    EZ = 1.44*h; EY = -0.085*w; EX = 0.034*w
    MZ = 1.38*h; MY = -0.090*w
    BZ = 1.47*h; BY = -0.082*w
    NZ = 1.41*h; NY = -0.094*w
    CZ = 1.42*h; CY = -0.082*w
    ...
    def _bw(co): return Vector((co.x * 1.05, co.y * 1.05, co.z))
    add_sk("Body_Weight", _bw)
    def _bm(co):
        if co.z > 0.90 * h: return Vector((co.x * 1.04, co.y * 1.04, co.z))
        return co
    add_sk("Body_Muscle", _bm)
    def _bs(co):
        if 1.08*h < co.z < 1.24*h: return Vector((co.x * 1.10, co.y, co.z))
        return co
    add_sk("Body_Shoulders", _bs)
    def _fw(co):
        if co.z > 1.30 * h: return Vector((co.x * 1.06, co.y, co.z))
        return co
    add_sk("Face_Width", _fw)
    def _fj(co):
        if 1.32*h < co.z < 1.40*h: return Vector((co.x * 1.08, co.y, co.z))
        return co
    add_sk("Face_Jaw", _fj)
...
def _smooth_noise(uv, scale, seed=0):
    ...
    def h(ix, iy):
        ...
        return g - np.floor(g)
    a = h(xi, yi); b = h(xi + 1, yi)
    cc = h(xi, yi + 1); d = h(xi + 1, yi + 1)
...
def create_procedural_textures(config):
    ...
    sr = config["skin_roughness"]; sm = config["skin_metallic"]
    ...
    bc = np.dstack([np.clip(sc[0]+n, 0, 1), np.clip(sc[1]+n, 0, 1),
                     np.clip(sc[2]+n, 0, 1), ones]).flatten()
...
def create_materials(body_obj, eye_objs, config):
    em = config.get("emissive"); tr = config.get("transmission", 0.0)
    textures = create_procedural_textures(config)
    skin = bpy.data.materials.new("Skin"); skin.use_nodes = True
    nt = skin.node_tree; nodes = nt.nodes; links = nt.links
    bsdf = nodes.get("Principled BSDF")
    if bsdf:
        if em:
            for key in ("Emission Color", "Emission"):
                if key in bsdf.inputs: bsdf.inputs[key].default_value = (*em, 1.0); break
            set_input(bsdf, "Emission Strength", 0.4)
        if tr > 0:
            for key in ("Transmission Weight", "Transmission"):
                if key in bsdf.inputs: bsdf.inputs[key].default_value = tr; break
    def tex_node(img, cs, x, y):
        n = nodes.new("ShaderNodeTexImage")
        n.image = img; n.image.colorspace_settings.name = cs
        n.location = (x, y); return n
    ...
    nmap = nodes.new("ShaderNodeNormalMap"); nmap.location = (-500, -250)
    links.new(norm_tex.outputs["Color"], nmap.inputs["Color"])
    links.new(nmap.outputs["Normal"], bsdf.inputs["Normal"])
    body_obj.data.materials.append(skin)
    ec = config["eye_color"]
    eye_mat = bpy.data.materials.new("Eyes"); eye_mat.use_nodes = True
    ebsdf = eye_mat.node_tree.nodes.get("Principled BSDF")
    if ebsdf:
        set_input(ebsdf, "Base Color", (*ec, 1.0))
        set_input(ebsdf, "Roughness", 0.12); set_input(ebsdf, "Metallic", 0.0)
        if em:
            for key in ("Emission Color", "Emission"):
                if key in ebsdf.inputs: ebsdf.inputs[key].default_value = (*ec, 1.0); break
...
def _kf(armature, bone_name, frame, rot=(0, 0, 0)):
...
def _keyframe_idle(arm, cfg):
    for fr in (0, 90):
        _kf(arm, "Spine", fr, (0.02, 0, 0)); _kf(arm, "Spine1", fr, (0.01, 0, 0))
        _kf(arm, "LeftArm", fr, (0, 0, 0.08)); _kf(arm, "RightArm", fr, (0, 0, -0.08))
        _kf(arm, "LeftForeArm", fr, (-0.05, 0, 0)); _kf(arm, "RightForeArm", fr, (-0.05, 0, 0))
        _kf(arm, "Head", fr, (0, 0, 0))
    _kf(arm, "Spine", 22, (-0.01, 0, 0)); _kf(arm, "Spine1", 22, (-0.02, 0, 0))
    _kf(arm, "Head", 22, (-0.01, 0.02, 0))
    _kf(arm, "Spine", 45, (0.04, 0, 0)); _kf(arm, "Spine1", 45, (0.02, 0, 0))
    _kf(arm, "Head", 45, (0.01, -0.01, 0))
    _kf(arm, "Spine", 67, (0.02, 0, -0.01)); _kf(arm, "Head", 67, (0, 0.03, 0))

def _keyframe_speaking(arm, cfg):
    for fr in (0, 120):
        _kf(arm, "Spine", fr, (0.03, 0, 0)); _kf(arm, "Spine1", fr, (0.02, 0, 0))
        _kf(arm, "LeftArm", fr, (0, 0, 0.10)); _kf(arm, "RightArm", fr, (0, 0, -0.10))
        _kf(arm, "LeftForeArm", fr, (-0.10, 0, 0)); _kf(arm, "RightForeArm", fr, (-0.10, 0, 0))
        _kf(arm, "Head", fr, (0, 0, 0))
    _kf(arm, "RightArm", 30, (0, 0, -0.35)); _kf(arm, "RightForeArm", 30, (-0.30, 0, 0))
    _kf(arm, "Head", 30, (0.05, -0.08, 0)); _kf(arm, "Spine2", 30, (0, 0.05, 0))
    _kf(arm, "LeftArm", 60, (0, 0, 0.35)); _kf(arm, "LeftForeArm", 60, (-0.30, 0, 0))
    _kf(arm, "Head", 60, (0.05, 0.08, 0)); _kf(arm, "Spine2", 60, (0, -0.05, 0))
    _kf(arm, "LeftArm", 90, (0, 0, 0.20)); _kf(arm, "RightArm", 90, (0, 0, -0.20))
    _kf(arm, "Head", 90, (0.08, 0, 0))

def _keyframe_thinking(arm, cfg):
    for fr in (0, 90):
        _kf(arm, "Spine", fr, (0.03, 0, 0)); _kf(arm, "Head", fr, (0.10, 0.10, 0))
        _kf(arm, "RightArm", fr, (0, 0, -0.08)); _kf(arm, "RightForeArm", fr, (-0.05, 0, 0))
    _kf(arm, "RightArm", 30, (-0.30, 0, -0.50)); _kf(arm, "RightForeArm", 30, (-1.10, 0.30, 0))
    _kf(arm, "Head", 30, (0.15, 0.05, 0))
    _kf(arm, "RightArm", 60, (-0.20, 0, -0.40)); _kf(arm, "RightForeArm", 60, (-0.90, 0.20, 0))
    _kf(arm, "Head", 60, (0.12, 0.08, 0))

def _keyframe_wave(arm, cfg):
    for fr in (0, 60):
        _kf(arm, "RightArm", fr, (0, 0, -0.08)); _kf(arm, "RightForeArm", fr, (-0.05, 0, 0))
        _kf(arm, "Head", fr, (0, 0, 0))
    _kf(arm, "RightArm", 15, (-1.20, 0, -0.30)); _kf(arm, "RightForeArm", 15, (-0.30, 0, 0))
    _kf(arm, "Head", 15, (0, -0.10, 0))
    _kf(arm, "RightForeArm", 30, (-0.30, 0.50, 0))
...
def create_animations(armature, config):
    ...
    for clip_name, kfn in [("idle", _keyframe_idle), ("speaking", _keyframe_speaking),
                            ("thinking", _keyframe_thinking), ("wave", _keyframe_wave)]:
...
def export_glb(filepath):
    bpy.ops.export_scene.gltf(
        filepath=str(filepath), export_format="GLB", export_yup=True,
        ...
        export_morph_tangent=False, export_morph_animation=False,
    )
...
def verify_glb(filepath):
    ...
    if len(data) < 20 or int.from_bytes(data[0:4], "little") != 0x46546C67:
        print(f"  ERROR: not a valid GLB"); return
    json_len = int.from_bytes(data[12:16], "little")
    js = data[20:20+json_len].decode("utf-8", errors="ignore").rstrip("\x00 \n\r,")
    gltf = json.loads(js)
    meshes = gltf.get("meshes", []); animations = gltf.get("animations", [])
    skins = gltf.get("skins", []); accessors = gltf.get("accessors", [])
    morph_names = set()
    for m in meshes:
...
def generate_species(name):
    ...
    mod = body.modifiers.new("Armature", "ARMATURE"); mod.object = armature
