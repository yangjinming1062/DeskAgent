...
**核心不变量**：云端永不接触用户本机与凭证，本地 Runner 零凭证运行，Client 是唯一的可信枢纽。即便伙伴人格被 prompt 注入攻陷，最坏情况也只是受限用户账户下的本地工具被调用，不会泄露云端凭证或影响其他用户。用户**设置**（非机密部分）云端为真源、Client 持镜像（[PROTOCOL.md §2.4](PROTOCOL.md)）——云端托管的是用户偏好数据而非本机访问面，机密与设备相关节（terminal、spiritagent）仅存本机。
...
## 2. 架构拓扑
    ...
    └─────────────────────────┬───────────────────────────────────┘
                              ...
                              │ /api/chat/ws
    ┌─────────────────────────▼───────────────────────────────────┐
    │ Client：桌面伙伴渲染 + 可信本地枢纽（内部边界见模块文档）     │
    └─────────────────────────┬───────────────────────────────────┘
...
| **状态持有** | 伙伴角色定义、形象资产、长期记忆、PostgreSQL、LLM API 凭证、会话历史、用户设置（真源） | 用户身份凭证（加密）、设置镜像（desktop-settings.json，含仅本机的机密节）、Runner 进程句柄、本地工具集 Schema 缓存、形象资产本地缓存 | 终端环境快照、浏览器会话 |
...
- **Backend 只持久化生效值**：供服务端闸门（主动消息、cron 自主回合、情绪/空间推理入口）使用——既不独立推导，也不存储用户偏好或焦点上下文；重启不丢档位。
...
- **affect 与文本同帧；语音合成按通道分双路径**：对话响应帧内联 {文本, 情绪}，不内联语音音频。会话外一次性语音（IM 语音条转写、气泡朗读、音色试听）由客户端在文本到达后拉取式合成——先据情绪切情绪状态、再据文本拉语音切说话状态，保证情绪基调先于语音进入；会话内实时语音通话走服务端编排的独立语音通道（[PROTOCOL.md §1.7](PROTOCOL.md)）——服务端把流式回复按句切分、逐句合成推送音频段，客户端顺序播放。语音通话回合事件只走语音通道、不进聊天网关 outbox，但对话历史与文字聊天落同一会话行。
...
10. **伙伴表达永不空白**：任何动画状态或情绪 cue 无对应就绪资产时，客户端必须按 [DESIGN.md §1.2](DESIGN.md) 的产品兜底策略降级，用户永远不可见空白或加载态。
