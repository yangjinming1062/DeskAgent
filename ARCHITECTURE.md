...
外部 IM（微信 iLink / QQ OneBot / 回环测试）经 **web 进程内**的通道桥（`services/channels/`）与同一伙伴对话——与调度器同为进程内 asyncio 任务，天然满足 §5.3 的单副本语义，不需要 omp-wechat 类方案的多进程单例锁。与 cron 自主回合的路由差异是本节的关键不变量：
...
- **affect 与文本同帧；语音合成按通道分双路径**：对话响应帧内联 {文本, 情绪}，不内联语音音频。会话外一次性语音（IM 语音条转写、气泡朗读、音色试听）由客户端在文本到达后拉取式合成——先据情绪切情绪状态、再据文本拉语音切说话状态，保证情绪基调先于语音进入；会话内实时语音通话走服务端编排的独立语音通道（[PROTOCOL.md §1.7](PROTOCOL.md)）——全双工链路：上行 PCM 常开，服务端 VAD 断句触发转写与回合，回复按子句/句成段、流式合成分块推送，客户端按块无缝播放；打断为双层（客户端乐观停播 + 服务端插话判别权威取消）。语音通话回合事件只走语音通道、不进聊天网关 outbox，但对话历史与文字聊天落同一会话行。
