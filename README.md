...
* **表情头像系统**：聊天气泡里“什么表情”由按情绪标签自动生成、缓存的头像情绪图承载——新情绪首次用到才生成，情绪表达永不空白。
...
* **始终语音或文字回复**：在「始终语音」模式下，回复文本在增量生成阶段由切分器按句切分，逐句流式合成"边生成边开口"、首子句成型即起播；随时可打断切回文字。默认文字模式则静默输出气泡，由你点播放按钮触发朗读。
