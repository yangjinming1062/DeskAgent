...
SpiritAgent 区别于传统桌面宠物或纯聊天插件的核心，在于伙伴拥有**从无到有的生成式诞生闭环**。首次安装时它是一颗温润呼吸的“蛋”；在完成一段自然温暖的对话后，系统即时生成专属半身像；随后并发生成多风格全身立绘样图供用户选择与微调；正面立绘确认后自动派生多视图并进入图生 3D 管线，赋予伙伴专属的 3D 骨骼模型与相册资产：
